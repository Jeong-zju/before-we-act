...
**UTC `2026-08-08T04:41Z` 过夜终态与 phase-balanced 恢复。** 第一轮 promotion 产生了真实但尚未跨越正式门的信号：e28 expert validation=`2/20 vs 1/20`、`PASSED`，随后 e29 formal Stack=`1/20 vs 3/20`、protected=`74`、total=`75/100`，`FAILED`；e25 RETAIN discovery=`2/20 vs 1/20`、`PASSED`，e35 validation=`1/20 vs 1/20`、`FAILED`；e26 robust reactive discovery=`3/20 vs 1/20`、`PASSED`。其余 e33 stochastic validation=`1/20 vs 1/20`、e19 BID discovery=`1/20 vs 1/20`、e24 PACE discovery=`0/20 vs 1/20`、e39 expert+stochastic discovery=`1/20 vs 1/20`、e42 expert+PACE discovery=`1/20 vs 1/20`，均按严格门 `FAILED`。这些结果说明多条路线未完成综合目标，但不是“整个问题已证明无解”。
...
**UTC `2026-08-08T08:54Z` operator-requested 全量资源清理。** 用户随后明确要求终止运行并不可恢复地清除历史分支/checkpoint，因此本条覆盖上一段“持续运行”状态。清理前先杀死 e54 promotion handoff，向 e54 独立进程组发送 INT，再删除其 producer tmux；e54 当时只完成 discovery `11/20`、success=`1`、终态链 `B/A/C=7/2/1`，未跑满 20 seeds，故只能记为 `STOPPED`，不能记为模型 `FAILED/PASSED`。结构化状态现为 `state=STOPPED, stage=operator_cleanup, program=none, pid/child_pid=0, exit_code=130`，更新时间 `08:54:08Z`；e55/e56 从未创建。随后终止所有可识别的 BWA 训练、验证、handoff、monitor 和全部 GPU compute。tmux 已不存在内部 session id `$0`，因此先保留最早且唯一人工入口 `ssh_tmux:0`，再把 session 显式重命名为 `0`；最终只存在 `session=0, window=0, pane=bash`。其余 8 个 session、所有非 0 window 均删除，保留窗口中的旧 BWA monitor 子进程也已终止。

checkpoint 清理先按验证后的精确命名文件集合执行：服务器 `/workspace` 在排除 dataset/HF cache/venv 后删除 `239` 个 checkpoint、`80,350,860,929 bytes`（`74.833 GiB`）；本地 `/home/jeong/zeno/wam` 删除 `5` 个历史 checkpoint、`6,631,937,036 bytes`。二次全扩展名审计又发现文件名不含 `checkpoint`、但位于 `checkpoints/` 的 `latent_wam/vision_source_weights` 等权重，以及 recovery/phase-head/untrained 权重；本地继续清空 `4` 个 checkpoint 目录中的 `14,754` 个文件、`65,287,893,656 bytes`（`60.804 GiB`），服务器继续删除 `107` 个 checkpoint 目录和 `8` 个非 cache 权重、`984,616,025 bytes`（`0.917 GiB`）。最终 named-checkpoint、checkpoint-directory file 和非 cache model-weight 三类复核均为 `0`；服务器磁盘由 `883 GiB used / 142 GiB available / 87%` 变为 `807 GiB used / 218 GiB available / 79%`。数据集、Hugging Face cache、明确命名为 `*_cache*.pt` 的特征/观测 cache、通用 DINO foundation、结构化 JSON/日志和已下载 MP4 没有删除，避免把“历史 checkpoint”扩大为未经点名的数据资产。
...
**UTC `2026-08-08T09:05Z` operator-selected winner merge。** 用户在已知 e53 formal=`2/20 < W12 3/20`、e54 仅完成 `11/20` 后仍明确要求合并 winner；因此本轮把当前唯一保留候选 `bwa/r15-role-query-view-dedup@bd1ae1e9f2f193c29cc851b529a36965d4df7aa0` 定义为 **operator-selected implementation winner**，不追溯改写为 Gate20 PASSED。该分支以 merge commit `03fda16` 合入 `feat/model-improvements`，保留 role-conditioned spatial query、bit-exact identical-view dedup、phase-balanced continuation 及其独立训练/验收脚本。另一条 evolution 分支只按时间顺序 cherry-pick `28` 个仅修改 `docs/` 的提交；其已淘汰 world-reactive/phase-routed 等模型代码没有混入 winner。合并后本地完整 `tests/before_we_act` 为 `118 passed, 10 warnings`，warnings 均为既有 Transformer nested-tensor 提示；修改 Python compile、全部 `*r15*.sh` 的 `bash -n` 和 `git diff --check` 均通过。服务器首次全测为 `116 passed/2 failed`，两项只因测试硬编码的 `/workspace/fe-pc-wam/.venv/bin/python` 不存在；临时把该路径链接到实际 `/venv/robofactory-act` 后不改代码重跑为 `118 passed, 5 warnings`，随后立即移除链接，远端 compile/shell/diff 同样通过。`feat/model-improvements` 推送后，两端 `bwa/r15-role-query-view-dedup` 与 `bwa/r15-closed-loop-evolution` worktree/local branch 及 GitHub branch 均删除，并再次 fetch-prune/reflog-expire/GC；GitHub 默认分支和唯一 branch 均为 `feat/model-improvements`。
...
# 查看已经持久运行的 30 秒刷新队列 monitor（Ctrl-b d 仅脱离，不停止）
ssh -t -p 10328 root@69.176.92.104 'tmux attach-session -t bwa-r15-monitor-dedup-queue'
