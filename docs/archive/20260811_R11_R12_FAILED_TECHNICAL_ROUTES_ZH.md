# R11/R12 失败技术路线完整归档（原 P1 路线图 V6.2）

> 归档日期：2026-08-11
> 原活动文档来源：`feat/model-improvements@96262bd937a805a18056d37928892ffe7f4a7f43`
> 终局：R11 正式 `NO_WINNER`；R12 按用户决策关闭为 `ARCHIVED_NO_WINNER`，两阶段均未向 baseline 合并代码或 checkpoint
> 事实边界：R12 Measurement 与 F0/F1 通过不等于候选成功；旧裁剪诊断轮的 L0/L1 在 Discovery gate 失败，L2 未形成 gate 终态，L3 未进入 Discovery，修正版正式重跑未完成。不得把阶段整体失败改写成 L2/L3 已由实验门槛判败
> 以下保留归档前路线图全文，用于审计设计、命令、门槛和当时状态；不再是活动执行指令
