...
闭环已经完整跑完。先说最重要的结果：按闭环前冻结的离线规则选出的 seed `20260817`，在与历史基线完全相同的 120 局协议上成功 `111/120`（`92.5%`）。W10 是 `88/120`（`73.3%`），B0-H 是 `95/120`（`79.2%`）；因此 3-N2 分别多成功 `23` 局和 `16` 局，即提升 `19.17` 和 `13.33` 个百分点。若只看第 12 节的数字门槛，它全部达到；但这是 N2 负责人诊断，不是从共同 base 重训的 N4，机器字段必须且确实保持 `formal_pass=false`。

| 任务（每项 20 局） | W10 | B0-H | 3-N2 | N2 相对 W10 | N2 相对 B0-H |
|---|---:|---:|---:|---:|---:|
| lift barrier | 20 | 20 | 20 | 0 | 0 |
| camera alignment | 8 | 5 | 14 | +6 | +9 |
| long pipeline delivery | 20 | 18 | 19 | -1 | +1 |
| take photo | 20 | 19 | 19 | -1 | 0 |
| pass shoe | 20 | 17 | 19 | -1 | +2 |
| place food | 0 | 16 | 20 | +20 | +4 |
| **合计** | **88** | **95** | **111** | **+23** | **+16** |

这不是靠一个任务掩盖另一个任务：3-N2 对 B0-H 六项里四项更好、两项持平，没有任何任务退步。最大的新增价值来自两个原本困难而且性质不同的任务：camera 从 B0-H 的 `5/20` 提到 `14/20`，place food 从 `16/20` 提到 `20/20`。相对 W10，N2 用 long/photo/shoe 各少一局的代价，换来 camera 多六局、food 多二十局；它保留了原有强项的大部分能力，同时补上了 W10 完全不会做的 food。

小样本 Validation5 也给出同方向结果，而不是只有选中 seed 偶然变好。B0-H 为 `24/30`，三个预注册 N2 seed 分别为 `26/30、26/30、27/30`；原冻结方向门的总体成功、强任务保护和等待代理全部通过。B0-H 的 paired inactivity rate 是 `56.23%`，三颗 N2 分别降到 `52.32%/50.95%/49.61%`。Validation20 的 N2 inactivity rate 为 `46.46%`，但历史 W10/B0-H 没冻结同一代理，所以这里只留作 N2 自身诊断，不能制造跨基线的伪比较。
