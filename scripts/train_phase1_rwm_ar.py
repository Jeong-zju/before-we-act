"""Train the Phase 1 recurrent world-action model (RWM-AR)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_paths["train"] or not split_paths["validation"]:
        raise RuntimeError("Phase 1 requires non-empty train and validation splits")
    ...
    metrics: dict[str, Any] = {
        "format_version": "wam.phase1.training/1",
        "curriculum": [],
    }
...
def _validate_settings(settings: Mapping[str, Any]) -> None:
    ...
    if settings["ensemble_size"] != 1:
        raise ValueError(
            "Phase 1 requires ensemble_size=1; ensemble belongs to Phase 2"
        )
    if not settings["predict_delta"]:
        raise ValueError("Phase 1 requires state_features.predict_delta=true")
    if settings["training_mode"] != "autoregressive":
        raise ValueError("Phase 1 requires training.mode=autoregressive")
    if settings["privileged_inputs"]:
        raise ValueError("Phase 1 inputs cannot include privileged state")
