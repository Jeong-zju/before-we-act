"""Audit every Phase 0 Gate A requirement for a proprioceptive WAM dataset."""
...
from policies.collection import PHASE0_BEHAVIOR_WEIGHTS  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fail unless a wam.proprio/1.0 dataset passes every Gate A check."
    )
...
def _audit_episode(path: Path, *, state_dim: int, action_dim: int) -> dict[str, Any]:
    with h5py.File(path, "r") as file:
        ...
        if behavior_id not in dict(PHASE0_BEHAVIOR_WEIGHTS):
...
def _behavior_check(
    ...
    expected = {
        behavior_id: weight / 100.0 for behavior_id, weight in PHASE0_BEHAVIOR_WEIGHTS
    }
    actual = {
