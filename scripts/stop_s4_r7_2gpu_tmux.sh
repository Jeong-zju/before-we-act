...
if [[ -z "${RUN_ROOT}" ]]; then
  RUN_ROOT="${FE_ROOT}/outputs/s4_r7_runs/${RUN_ID}"
fi
RUN_ROOT="$(realpath -m "${RUN_ROOT}")" || fail "cannot resolve run root"
MANIFEST="${RUN_ROOT}/run_manifest.json"
if [[ ! -f "${MANIFEST}" ]]; then
  fail "missing S4-R7 run manifest: ${MANIFEST}"
fi
...
if [[ "${MANIFEST_FORMAT}" != wam.robofactory.s4_r7.runtime/1 || \
      "${MANIFEST_ROUND}" != s4-r7 || "${MANIFEST_RUN_ID}" != "${RUN_ID}" ]]; then
  fail "manifest identity does not match requested S4-R7 run ${RUN_ID}"
fi
...
for name in "${TARGET_WINDOWS[@]}"; do
  if [[ "${CURRENT_WINDOW}" == "${name}" ]]; then
    fail "run stop from a non-S4-R7 window; current target window is ${name}"
  fi
done
...
run_process_ids() {
  ...
  for environment in "${paths[@]}"; do
    ...
    if grep -Fzqx "S4_R7_RUN_ROOT=${RUN_ROOT}" "${environment}" 2>/dev/null; then
      printf '%s\n' "${pid}"
    fi
  done
}

printf 'S4-R7 stop target (exact run identity):\n'
printf '  run id: %s\n  run root: %s\n  tmux session: %s\n' \
...
mapfile -t remaining < <(run_process_ids)
if (( ${#remaining[@]} > 0 )); then
  printf 'Sending SIGTERM only to tagged S4-R7 processes: %s\n' "${remaining[*]}"
...
mapfile -t remaining < <(run_process_ids)
if (( ${#remaining[@]} > 0 )); then
  printf >&2 'Failed to terminate tagged S4-R7 processes: %s\n' "${remaining[*]}"
  exit 5
fi
for name in "${TARGET_WINDOWS[@]}"; do
  if tmux_window_id "${name}" >/dev/null; then
    printf >&2 'Failed to close S4-R7 tmux window: %s\n' "${name}"
    exit 5
  fi
done

printf 'Stopped only S4-R7 run %s and closed its four windows.\n' "${RUN_ID}"
