...
exec /workspace/venvs/robofactory-act/bin/python -u /workspace/act_liftbarrier/train_pair_route_single_b40_120k.py \
  ...
  --output runs/strict640x480_v2/results/pair_route_spec_only_single_b40_120k \
  --updates 120000 --batch-size 40 --save-updates 60000,80000,100000,120000 \
  --sampler-mode weighted_items --workers 8 \
  --relation-weight 0 --anchor-weight 0 --capability-weight 0 \
  --specialization-weight 0.01 --counterfactual-every 4 \
  --experiment-label pair_route_specialization_only --gpu-label GPU2
