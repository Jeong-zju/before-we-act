#!/usr/bin/env python3
"""Persistent status, heartbeat and special-gate monitor for S4-R8.

Candidate and prepare wrappers use the small CLI in this module to publish
atomic status files.  The monitor deliberately derives the R8 structural,
causal and prefix gates from their individual metrics; a generic ``passed``
...
FORMAT_VERSION = "wam.robofactory.s4_r8.runtime/1"
CANDIDATES = ("P0", "P1")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    status = commands.add_parser(
        "status", help="publish candidate status and heartbeat"
    )
    ...
    shared = commands.add_parser(
        "shared-status", help="publish prepare status and heartbeat"
    )
    ...
    heartbeat = commands.add_parser(
        "heartbeat", help="refresh one heartbeat atomically"
    )
    ...
    monitor_parser = commands.add_parser(
        "monitor", help="render the persistent monitor"
    )
...
def _add_status_arguments(
    parser: argparse.ArgumentParser, *, include_gpu: bool
) -> None:
    ...
    _atomic_json(
        manifest,
        {
            ...
            "validation_order": [
                ...
                "shuffle_shared",
                "prefix_suffix_exact",
                "prefix_shuffle_by_source_horizon",
            ],
        },
    )
...
def render_monitor(root: Path) -> str:
    ...
    lines = [
        (
            f"WAM S4-R8 monitor | run={manifest.get('run_id', root.name)} | "
...
def _future_cache_progress_lines(root: Path) -> list[str]:
    ...
    for line in reversed(data.decode("utf-8", errors="replace").splitlines()):
        ...
        if (
            ...
            or value.get("event") != "future_cache_progress"
        ):
            continue
    ...
    if not latest:
        return [
            "future cache workers | starting; shared heartbeat remains authoritative"
        ]
    parts = []
    for worker in (0, 1):
...
def _candidate_lines(
    ...
    flow_state = progress.get("flow_unfreeze_state") or (
        ...
        else f"frozen->{FLOW_UNFREEZE_UPDATE}"
    )
...
def _special_acceptance_lines(root: Path) -> list[str]:
    lines = [
        "R8 special acceptance (derived gates; a generic passed=true is insufficient):"
    ]
    ...
    for candidate in CANDIDATES:
        ...
        suffix_report = _first_json(
            candidate_root / "validation" / "prefix_suffix_exact.json"
        )
        prefix_report = _first_json(
            candidate_root / "validation" / "prefix_shuffle_by_source_horizon.json"
        )
        ...
        suffix_groups = _mapping_or_empty(suffix_report.get("groups"))
        prefix_groups = _mapping_or_empty(prefix_report.get("groups"))
        suffix_exact = sum(
        ...
        prefix_positive = sum(
            isinstance(value, Mapping)
            and _number(value.get("episode_bootstrap_95_lower")) is not None
            and float(value["episode_bootstrap_95_lower"]) > 0.0
            for value in prefix_groups.values()
        )
        minimum_lower = min(
            (
                float(value["episode_bootstrap_95_lower"])
                for value in prefix_groups.values()
        ...
        lines.append(
            f"  {candidate} R8 prefix causality | "
            f"suffix+prefix-sensitive={suffix_exact}/12 "
            f"bootstrap-lower>0={prefix_positive}/12 "
            f"min-lower={_number_text(minimum_lower)}"
        )
        utility_values = _mapping_or_empty(candidate_report.get("utility_calibration"))
        lines.append(
            f"  {candidate} R7 axis isolation | "
            f"WUC={_number_text(utility_values.get('utility_coupling_weight'))} "
            f"backward-disabled={utility_values.get('wuc_backward_disabled', '?')}"
        )
    ...
    lines.append(
        ...
        + (
            ...
            f"method-combination={final.get('method_combination_not_weight_fusion', '?')} "
...
def _gate20_from_disk(candidate_root: Path) -> Mapping[str, Any]:
    ...
    for condition in (*CORE_CONDITIONS, *DIAGNOSTIC_CONDITIONS):
        summary = _first_json(
            candidate_root / "validation" / "gate20" / condition / "gate_summary.json"
        )
...
def _causal_gate(
    ...
    return (
        "PASS" if normal >= legacy and normal > zero and normal > shuffled else "FAIL"
    )
