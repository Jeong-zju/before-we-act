...
# Run one S4-R8 candidate.  Every fallible operation is checked explicitly so
# failures remain visible in the permanent tmux window and runtime monitor.

FE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RUNNER_NAME="run_s4_r8_candidate.sh"
RUN_ID=""
RUN_ROOT_INPUT=""
READY_FILE_INPUT=""
FAILED_FILE_INPUT=""
...
fail_early() {
  local message="$*"
  printf >&2 'S4-R8 candidate runner error: %s\n' "${message}"
...
case "${CANDIDATE}" in
  P0)
    EXPECTED_GPU=0
    EXPECTED_BRANCH="s4/r8-p0-horizon-prefix-mean"
    EXPECTED_MODEL_KIND="s4_r8_horizon_prefix_mean"
    ;;
  P1)
    EXPECTED_GPU=1
    EXPECTED_BRANCH="s4/r8-p1-causal-prefix-attention"
    EXPECTED_MODEL_KIND="s4_r8_causal_prefix_attention"
    ;;
  *)
    printf >&2 'Invalid S4-R8 candidate: %s\n' "${CANDIDATE}"
    exit 2
    ;;
esac
...
if [[ "${HEARTBEAT_SECONDS}" != "20" ]]; then
  printf >&2 'S4-R8 heartbeat interval is fixed at 20 seconds, got %s\n' \
    "${HEARTBEAT_SECONDS}"
  exit 2
fi
...
expected_branch = {
    "P0": "s4/r8-p0-horizon-prefix-mean",
    "P1": "s4/r8-p1-causal-prefix-attention",
}[candidate]
checks = {
    "format_version": manifest.get("format_version") == "wam.robofactory.s4_r8.runtime/1",
    "round_id": manifest.get("round_id") == "s4-r8",
    ...
    "config_path": config == worktree / "configs/wam_flow/s4_r8.yaml",
...
STATUS_TOOL="${BASE_REPO}/scripts/s4_r8_runtime.py"
if [[ ! -f "${STATUS_TOOL}" ]]; then
  fail_early "missing S4-R8 runtime status program: ${STATUS_TOOL}"
fi
...
P0_CONFIG="$(realpath -e -- "${P0_WORKTREE}/configs/wam_flow/s4_r8.yaml")" || \
  fail_early "P0 config is unavailable"
P1_CONFIG="$(realpath -e -- "${P1_WORKTREE}/configs/wam_flow/s4_r8.yaml")" || \
...
ENV_FILE="${FE_ROOT}/experiments/wam_flow/s4_r8/candidate.env"
if [[ ! -f "${ENV_FILE}" ]]; then
...
candidate, kind, aggregator = validate_s4_r8_candidate(config)
...
expected_aggregator = (
    "prefix_mean" if expected_candidate == "P0" else "causal_prefix_attention"
)
if aggregator != expected_aggregator:
    raise ValueError("config aggregator axis differs from the S4-R8 registry")
if config.get("format_version") != "wam.robofactory.s4_r8.horizon_causal.config/1":
    raise ValueError("unsupported S4-R8 config format")
...
PAIR_VALIDATOR="${BASE_REPO}/scripts/validate_s4_r8_branch_pair.py"
ACCEPT_TOOL="${BASE_REPO}/scripts/accept_s4_r8.py"
TRAINER="${FE_ROOT}/scripts/train_s4_r8_horizon_causal.py"
EVALUATOR="${FE_ROOT}/scripts/evaluate_s4_r8_causal.py"
PREFIX_EVALUATOR="${FE_ROOT}/scripts/evaluate_s4_r8_prefix_causality.py"
for required_script in "${STATUS_TOOL}" "${PAIR_VALIDATOR}" "${ACCEPT_TOOL}"; do
  if [[ ! -f "${required_script}" ]]; then
    fail_early "missing shared S4-R8 control program: ${required_script}"
  fi
done
...
PREFIX_SUFFIX_REPORT="${VALIDATION_ROOT}/prefix_suffix_exact.json"
PREFIX_SHUFFLE_REPORT="${VALIDATION_ROOT}/prefix_shuffle_by_source_horizon.json"
VALIDATION_PROGRESS="${VALIDATION_ROOT}/progress.jsonl"
PAIR_EXACT="${PAIR_ROOT}/pair_exact.json"
STEP0_EXACT="${PAIR_ROOT}/p0_p1_step0_exact.json"
...
P0_PREFIX_SUFFIX="${P0_ROOT}/validation/prefix_suffix_exact.json"
P1_PREFIX_SUFFIX="${P1_ROOT}/validation/prefix_suffix_exact.json"
P0_PREFIX_SHUFFLE="${P0_ROOT}/validation/prefix_shuffle_by_source_horizon.json"
P1_PREFIX_SHUFFLE="${P1_ROOT}/validation/prefix_shuffle_by_source_horizon.json"
...
verify_preflight() {
  ...
  local mode="${3:-pass}"
  local expected_kind
  local expected_aggregator
  local expected_config_sha
  if [[ "${candidate}" == "P0" ]]; then
    expected_kind="s4_r8_horizon_prefix_mean"
    expected_aggregator="prefix_mean"
    expected_config_sha="${P0_CONFIG_SHA256}"
  else
    expected_kind="s4_r8_causal_prefix_attention"
    expected_aggregator="causal_prefix_attention"
    expected_config_sha="${P1_CONFIG_SHA256}"
  fi
  python3 - "${path}" "${candidate}" "${expected_kind}" \
    "${expected_config_sha}" "${MICRO_TEAM_BATCH}" \
    "${GRADIENT_ACCUMULATION}" "${mode}" "${expected_aggregator}" <<'PY'
import json
import math
from pathlib import Path
import re
...
path, candidate, expected_kind, config_sha256, micro_raw, accumulation_raw, mode, expected_aggregator = sys.argv[1:]
...
if (
    value.get("format_version") != "wam.robofactory.s4_r8.preflight/1"
    or identity.get("round_id") != "s4-r8"
    or identity.get("candidate_id") != candidate
    or identity.get("model_kind") != expected_kind
    or identity.get("action_prefix_aggregator") != expected_aggregator
...
expected = {
    "format_version": "wam.robofactory.s4_r8.preflight_provenance/1",
    "round_id": "s4-r8",
    "run_id": run_id,
...
if (
    pair.get("format_version") != "wam.robofactory.s4_r8.pair_exact/1"
    or pair.get("round_id") != "s4-r8"
...
verify_checkpoint() {
  ...
  ( cd "${BASE_REPO}" && uv run --frozen python - "${path}" "${candidate}" \
      ...
      "${expected_config_sha}" "${expected_git_commit}" \
      "${expected_aggregator}" <<'PY'
...
path_raw, candidate, expected_kind, mode, total_raw, config_raw, config_sha, expected_git_commit, expected_aggregator = sys.argv[1:]
...
if mode == "complete":
    ...
    if value.get("format_version") != resume_format:
        raise ValueError("resume has an unsupported S4-R8 format")
    identity = value.get("identity")
if not isinstance(identity, Mapping):
    raise ValueError("checkpoint/resume identity must be a mapping")
if (
    identity.get("round_id") != "s4-r8"
    or identity.get("candidate_id") != candidate
    or identity.get("model_kind") != expected_kind
    or identity.get("action_prefix_aggregator") != expected_aggregator
    or float(identity.get("utility_coupling_weight", -1.0)) != 0.0
):
...
verify_candidate_report() {
  ...
  ( cd "${BASE_REPO}" && uv run --frozen python - "${report}" \
      "${checkpoint}" "${candidate}" "${suffix}" "${shuffle}" <<'PY'
from pathlib import Path
import sys

from scripts.accept_s4_r8 import _evaluate_candidate, _read_checkpoint, _read_json

report_path, checkpoint_path, candidate, suffix_path, shuffle_path = sys.argv[1:]
...
_evaluate_candidate(
    ...
    report,
    checkpoint,
    _read_json(Path(suffix_path)),
    _read_json(Path(shuffle_path)),
)
PY
  )
}
...
verify_acceptance() {
  ( cd "${BASE_REPO}" && uv run --frozen python - "${ACCEPTANCE}" \
      ...
      "${P1_CHECKPOINT}" "${P0_PREFIX_SUFFIX}" "${P1_PREFIX_SUFFIX}" \
      "${P0_PREFIX_SHUFFLE}" "${P1_PREFIX_SHUFFLE}" <<'PY'
from pathlib import Path
import sys

from scripts.accept_s4_r8 import build_acceptance, _read_checkpoint, _read_json
...
expected = build_acceptance(
    _read_json(pair_path),
    {
        "P0": (
            ...
            _read_json(p0_suffix_path),
            _read_json(p0_shuffle_path),
        ),
        "P1": (
            _read_json(p1_report_path),
            _read_checkpoint(p1_ckpt_path),
            _read_json(p1_suffix_path),
            _read_json(p1_shuffle_path),
        ),
    },
)
if observed.get("format_version") != "wam.robofactory.s4_r8.acceptance/1" or observed.get("round_id") != "s4-r8":
...
if ! grep -Fq 's4-r8 shared inputs verified' "${READY_FILE}"; then
  abort "shared.ready content is not an S4-R8 verification marker"
fi
...
expected = {
    (base / "artifacts/s4_r8_parent/legacy_r6l_policy.pt").resolve(strict=True),
    (base / "artifacts/s4_r8_parent/active_flow.pt").resolve(strict=True),
    (base / "artifacts/s4_r8_parent/local_future.pt").resolve(strict=True),
    (base / "artifacts/s4_r8_parent/team_future.pt").resolve(strict=True),
...
export S4_R8_SHARED_HDF5_RECEIPT="${SHARED_DATA_RECEIPT}"
export S4_R8_SHARED_HDF5_RECEIPT_SHA256="${SHARED_DATA_RECEIPT_SHA256}"
...
S4_R8_FUTURE_FEATURE_CACHE="$(jq -er '.root | strings' \
  "${FUTURE_CACHE_POINTER}")" || abort "future feature cache root is invalid"
S4_R8_FUTURE_FEATURE_CACHE="$(realpath -e -- \
  "${S4_R8_FUTURE_FEATURE_CACHE}")" || abort "future feature cache is unavailable"
S4_R8_FUTURE_FEATURE_CACHE_SHA256="$(jq -er \
  '.features_sha256 | strings' "${FUTURE_CACHE_POINTER}")" || \
  abort "future feature cache SHA256 is invalid"
if [[ ! "${S4_R8_FUTURE_FEATURE_CACHE_SHA256}" =~ ^[0-9a-f]{64}$ || \
      ! -f "${S4_R8_FUTURE_FEATURE_CACHE}/features.npy" || \
      "$(sha256sum "${S4_R8_FUTURE_FEATURE_CACHE}/features.npy" | awk '{print $1}')" != \
        "${S4_R8_FUTURE_FEATURE_CACHE_SHA256}" ]]; then
  abort "future feature cache binary differs after shared.ready"
fi
export S4_R8_FUTURE_FEATURE_CACHE S4_R8_FUTURE_FEATURE_CACHE_SHA256
...
ROBOFACTORY_ROOT="${S4_R8_ROBOFACTORY_ROOT:-$(dirname "${BASE_REPO}")/RoboFactory}"
RF_PYTHON="${S4_R8_RF_PYTHON:-${ROBOFACTORY_ROOT}/.venv/bin/python}"
S4_R8_ROBOFACTORY_ROOT="${ROBOFACTORY_ROOT}"
S4_R8_RF_PYTHON="${RF_PYTHON}"
LPD_POLICY_KIND=s4_flow
LPD_EXPERIMENT_SLUG="s4_r8_${SLUG}"
LPD_CONFIG="${CONFIG}"
...
export ROBOFACTORY_ROOT RF_PYTHON S4_R8_ROBOFACTORY_ROOT S4_R8_RF_PYTHON
...
if [[ -f "${PREFLIGHT_REPORT}" ]]; then
  ...
  if [[ ! -f "${PREFLIGHT_REPORT}" ]] || \
    ...
    if (( preflight_code != 0 )); then
      FAILURE_DETAIL="train_s4_r8_horizon_causal.py preflight exited ${preflight_code} without a valid terminal report"
      exit "${preflight_code}"
    fi
...
if [[ ! -f "${PAIR_EXACT}" ]]; then
  run_stage pair_validation validate_s4_r8_branch_pair.py \
...
if [[ "${PAIR_FALLBACK}" == "dino8_micro4_accum3" || \
  ...
  CURRENT_PROGRAM="validate_s4_r8_branch_pair.py"
...
if ! verify_pair_exact || ! ensure_digest "${PAIR_EXACT}" || \
   ! ensure_digest "${STEP0_EXACT}"; then
  flock -u "${PAIR_FD}" || true
...
if [[ -f "${CHECKPOINT}" ]]; then
  ...
  run_stage training train_s4_r8_horizon_causal.py \
  ...
  if (( training_code != 0 )); then
    FAILURE_DETAIL="train_s4_r8_horizon_causal.py formal training exited ${training_code}"
    exit "${training_code}"
  fi
  if [[ ! -f "${CHECKPOINT}" ]] || \
...
if [[ ! -f "${CANDIDATE_REPORT}" ]]; then
  ...
  run_stage validating evaluate_s4_r8_causal.py \
    "normal complete first; then legacy/gate-zero/shuffle-all core Gate20; diagnostics last" \
    uv run --frozen python scripts/evaluate_s4_r8_causal.py \
      --config "${CONFIG}" --checkpoint "${CHECKPOINT}" \
      --output "${CANDIDATE_REPORT}" --progress-log "${VALIDATION_PROGRESS}" \
      --device cuda:0
  evaluation_code=$?
  if (( evaluation_code != 0 )); then
    FAILURE_DETAIL="evaluate_s4_r8_causal.py exited ${evaluation_code}"
    exit "${evaluation_code}"
  fi
fi

if [[ ! -f "${PREFIX_SUFFIX_REPORT}" || \
      ! -f "${PREFIX_SHUFFLE_REPORT}" ]]; then
  if [[ ! -f "${PREFIX_EVALUATOR}" ]]; then
    abort "missing R8 prefix evaluator: ${PREFIX_EVALUATOR}"
  fi
  run_stage validating_prefix evaluate_s4_r8_prefix_causality.py \
    "checking all source×horizon suffix exactness and episode-bootstrap legal-prefix shuffle deltas" \
    uv run --frozen python scripts/evaluate_s4_r8_prefix_causality.py \
      --config "${CONFIG}" --checkpoint "${CHECKPOINT}" \
      --prefix-suffix-output "${PREFIX_SUFFIX_REPORT}" \
      --prefix-shuffle-output "${PREFIX_SHUFFLE_REPORT}" \
      --progress-log "${VALIDATION_PROGRESS}" --device cuda:0
  prefix_code=$?
  if (( prefix_code != 0 )); then
    FAILURE_DETAIL="evaluate_s4_r8_prefix_causality.py exited ${prefix_code}"
    exit "${prefix_code}"
  fi
fi

if [[ ! -f "${CANDIDATE_REPORT}" || ! -f "${PREFIX_SUFFIX_REPORT}" || \
      ! -f "${PREFIX_SHUFFLE_REPORT}" ]] || \
   ! verify_candidate_report "${CANDIDATE_REPORT}" "${CHECKPOINT}" \
      "${CANDIDATE}" "${PREFIX_SUFFIX_REPORT}" "${PREFIX_SHUFFLE_REPORT}" || \
   ! ensure_digest "${CANDIDATE_REPORT}" || \
   ! ensure_digest "${PREFIX_SUFFIX_REPORT}" || \
   ! ensure_digest "${PREFIX_SHUFFLE_REPORT}"; then
  abort "R8 evaluator outputs failed checkpoint-bound special acceptance validation"
fi
...
PEER_SUFFIX="${P1_PREFIX_SUFFIX}"
PEER_SHUFFLE="${P1_PREFIX_SHUFFLE}"
if [[ "${PEER}" == "P0" ]]; then
  PEER_SUFFIX="${P0_PREFIX_SUFFIX}"
  PEER_SHUFFLE="${P0_PREFIX_SHUFFLE}"
fi
wait_for_peer_file "${PEER_SUFFIX}" "${PEER}" waiting_peer_report \
  "waiting for ${PEER} suffix-exact report"
wait_for_peer_file "${PEER_SHUFFLE}" "${PEER}" waiting_peer_report \
  "waiting for ${PEER} prefix-shuffle bootstrap report"
if ! verify_pair_exact || \
   ...
   ! verify_candidate_report "${P0_REPORT}" "${P0_CHECKPOINT}" P0 \
      "${P0_PREFIX_SUFFIX}" "${P0_PREFIX_SHUFFLE}" || \
   ! verify_candidate_report "${P1_REPORT}" "${P1_CHECKPOINT}" P1 \
      "${P1_PREFIX_SUFFIX}" "${P1_PREFIX_SHUFFLE}" || \
   ! ensure_digest "${P0_REPORT}" || ! ensure_digest "${P1_REPORT}" || \
   ! ensure_digest "${P0_PREFIX_SUFFIX}" || \
   ! ensure_digest "${P1_PREFIX_SUFFIX}" || \
   ! ensure_digest "${P0_PREFIX_SHUFFLE}" || \
   ! ensure_digest "${P1_PREFIX_SHUFFLE}"; then
...
if [[ ! -f "${ACCEPTANCE}" ]]; then
  run_stage accepting accept_s4_r8.py \
    "applying R8 structural, causal, suffix-exact, prefix-bootstrap and paired Gate20 winner rules" \
    uv run --frozen python "${ACCEPT_TOOL}" --pair-exact "${PAIR_EXACT}" \
      ...
      --p0-prefix-suffix "${P0_PREFIX_SUFFIX}" \
      --p1-prefix-suffix "${P1_PREFIX_SUFFIX}" \
      --p0-prefix-shuffle "${P0_PREFIX_SHUFFLE}" \
      --p1-prefix-shuffle "${P1_PREFIX_SHUFFLE}" \
      --output "${ACCEPTANCE}"
  acceptance_code=$?
  if (( acceptance_code != 0 )); then
    flock -u "${ACCEPT_FD}" || true
    FAILURE_DETAIL="accept_s4_r8.py exited ${acceptance_code}"
...
COMPLETED=1
printf 'S4-R8 %s complete: decision=%s winner=%s root=%s\n' \
