...
# Run a Hugging Face network command with the exact interactive token, bounded
# exponential backoff, and an explicit Xet policy. The token is passed only in
# the child environment; it is never appended to argv.
hf_with_retry() {
  if (( $# < 3 )); then
  ...
  if [[ ! "${delay}" =~ ^[0-9]+$ ]]; then
    printf >&2 'HF_DOWNLOAD_INITIAL_BACKOFF_SECONDS must be a non-negative integer.\n'
    return 2
  fi
