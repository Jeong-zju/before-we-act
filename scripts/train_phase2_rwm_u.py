"""Train independent episode-bootstrap RWM-U members and Gate C ablation."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--phase1-checkpoint-dir", type=Path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_paths["train"] or not split_paths["validation"]:
        raise RuntimeError("Phase 2 requires non-empty train and validation splits")

    phase1_model, phase1_metadata = load_wam_checkpoint(
        settings["phase1_checkpoint_dir"],
        device="cpu",
        expected_schema_version=PROPRIO_WAM_SCHEMA_VERSION,
    )
    stats = phase1_metadata["normalization"]
    if (
        phase1_model.config.state_dim != settings["state_dim"]
        or phase1_model.config.action_dim != settings["action_dim"]
    ):
        raise ValueError("Phase 1 checkpoint dimensions do not match Phase 2 config")
    _validate_phase1_data_provenance(
        phase1_metadata["dataset_manifest"],
        split_paths,
    ...
    with TrainingProgress(
        ...
        try:
            ...
            training_metrics = {
                "format_version": "wam.phase2.training/1",
...
def _validate_settings(settings: Mapping[str, Any]) -> None:
    if settings["family"] != "rwm_u" or settings["ensemble_size"] < 2:
        raise ValueError("Phase 2 requires model.family=rwm_u and ensemble_size>=2")
    if not settings["bootstrap"]:
        raise ValueError("Phase 2 requires episode bootstrap sampling")
    if not settings["predict_delta"] or settings["training_mode"] != "autoregressive":
        raise ValueError("Phase 2 requires autoregressive delta prediction")
    if settings["privileged_inputs"]:
        raise ValueError("Phase 2 runtime inputs cannot include privileged state")
...
def _validate_phase1_data_provenance(
    ...
    *,
    ...
    if smoke_subset:
        return
    reference = phase1_manifest.get("partitions")
    if not isinstance(reference, Mapping):
        raise ValueError("Phase 1 checkpoint has no auditable dataset partitions")
    current = {
        name: [str(path.resolve()) for path in paths]
        for name, paths in split_paths.items()
    }
    ...
    if current != normalized_reference:
        raise ValueError(
            "Phase 2 data split differs from the Phase 1 normalization provenance; "
            "retrain Phase 1 or explicitly run a limited --max-episodes smoke test"
        )
...
def _save_partial_state(
    ...
    payload = {
        "format_version": "wam.phase2.partial/1",
...
def _provenance(
    ...
    return {
        ...
        "pid": os.getpid(),
        "external_checkpoints": [str(settings["phase1_checkpoint_dir"])],
    }
