"""Evaluate Phase 1 RWM-AR rollouts and emit an auditable Gate B report."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/wam/phase1_rwm_ar_v1.yaml",
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=Path,
        default=ROOT / "checkpoints/wam_cooperative_stop_v1",
    )
    parser.add_argument(
        "--phase0-output-dir",
        type=Path,
        default=ROOT / "outputs/wam_phase0_v2_mixed",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs/wam_phase1_open_loop_v1",
    )
...
def main(argv: list[str] | None = None) -> int:
        ...
        sorted(
            {
                *(int(item) for item in config["evaluation"]["open_loop_horizons"]),
                int(config["evaluation"]["gate_b_rollout_horizon"]),
            }
        )
    )
    ...
    phase0_mlp = _load_phase0_mlp(args.phase0_output_dir, device)
    ...
    with TrainingProgress(
        ...
        try:
            ...
            report = {
                "format_version": "wam.phase1.open_loop/1",
                "checkpoint": str(args.checkpoint_dir.resolve()),
                "phase0_reference": str(args.phase0_output_dir.resolve()),
            ...
            output_progress.finish(
                "Gate B " + ("passed" if gate_b["passed"] else "not passed")
            )
        finally:
    ...
    return 0 if gate_b["passed"] else 2
...
def _run_overfit_only(
    ...
    with TrainingProgress(
        ...
        report = {
            "format_version": "wam.phase1.overfit_audit/1",
            "overfit": result,
        }
...
def _evaluate_overfit_checkpoint(
    ...
    try:
        ...
        nrmse_limit = float(config["evaluation"]["gate_b_overfit_nrmse_max"])
        closed_limit = float(
            config["evaluation"]["gate_b_overfit_closed_rmse_max"]
        )
        return {
...
def _gate_b_report(
    ...
    *,
    ...
    beats_baselines = (
        ...
        and one_step["rwm_ar"] < one_step["phase0_mlp_recursive"]
    )
    stability = models["rwm_ar"]["exact_horizon"]["16"]
    violation_limit = float(config["evaluation"]["gate_b_constraint_violation_max"])
    stable = (
    ...
    checks = {
        ...
        "one_step_beats_constant_velocity_and_phase0_mlp": {
    ...
    return {
        ...
        "checks": checks,
        "note": "Gate B only; this is not a Gate C or Phase 2 uncertainty claim.",
    }
...
def _load_phase0_mlp(path: Path, device: torch.device) -> Phase0RecursiveBaseline:
    ...
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise RuntimeError(f"Phase 0 MLP strict load failed: {incompatible}")
    model.to(device).eval()
    return Phase0RecursiveBaseline(
        model=model,
...
def _write_rollout_svg(path: Path, example: Mapping[str, np.ndarray]) -> None:
    ...
    parts = [
        ...
        '<text x="72" y="34" fill="#e2e8f0" font-family="monospace" font-size="20">Phase 1 open-loop rollout</text>',
    ]
