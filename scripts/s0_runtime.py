#!/usr/bin/env python3
"""Create and monitor one four-candidate S0 reference run."""
...
def initialize_run(
    ...
    *,
    ...
    for item in worktrees:
        ...
        parsed_worktrees[candidate] = str(Path(path).expanduser().resolve())
    if set(parsed_worktrees) != set(CANDIDATES):
        raise ValueError("init requires exactly one worktree for B0, B1, B2 and B3")
    payload = {
...
def monitor_run(run_root: Path, *, interval: float, once: bool) -> None:
    ...
    try:
        ...
        print("\nMonitor stopped; tmux training windows were not interrupted.")
...
def collect_candidate(run_root: Path, candidate: str) -> dict[str, str]:
    ...
    gate_summaries = sorted(validation_root.rglob("gate_summary.json"))
    if gate_summaries:
        gate = _load_json(gate_summaries[-1])
...
def _gpu_lines() -> list[str]:
    ...
    if result.returncode != 0:
        detail = " ".join(result.stderr.split()) or "driver query failed"
        return [f"GPU: {detail}"]
