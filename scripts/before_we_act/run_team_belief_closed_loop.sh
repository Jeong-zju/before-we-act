...
SEED_ROOT="${BWA_TEAM_BELIEF_SEED_ROOT:-/workspace/bwa_runs/w10-six-task-v1/seeds/validation}"
...
[[ -f "${TEMPORAL_CHECKPOINT}" ]] || fail "B0-H checkpoint is missing"
...
if [[ ! -f "${AUTHORIZATION}" ]]; then
  ...
  "${PYTHON}" scripts/before_we_act/prepare_team_belief_closed_loop.py \
    --roadmap "${ROADMAP}" --contract "${CONTRACT}" --run-root "${RUN_ROOT}" \
    --old-conclusion "${OLD_CONCLUSION}" --temporal-checkpoint "${TEMPORAL_CHECKPOINT}" \
...
TEMPORAL_SHA="$(jq -r '.b0h_checkpoint_sha256' "${AUTHORIZATION}")"
run_model_validation b0h b0h "${TEMPORAL_CHECKPOINT}" "${TEMPORAL_SHA}" 3 & PIDS+=("$!")
for gpu in 0 1 2; do
  seed="${SEEDS[$gpu]}"
...
"${PYTHON}" scripts/before_we_act/summarize_team_belief_smoke_validation.py \
...
BWA_TEAM_BELIEF_CONCLUSION="${AUTHORIZATION}" \
...
trap - ERR
printf 'TEAM_BELIEF_CLOSED_LOOP_COMPLETED validation5=%s validation20=%s\n' \
