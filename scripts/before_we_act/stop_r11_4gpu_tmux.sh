...
RUN_ROOT=/workspace/bwa_runs/r11-current
SELECTION=all
GRACE=30
DRY_RUN=0
...
[[ "$(jq -r '.round' "$MANIFEST")" == R11 && "$(realpath -m "$(jq -r '.run_root' "$MANIFEST")")" == "$(realpath -m "$RUN_ROOT")" ]] || {
  printf 'run manifest identity mismatch\n' >&2; exit 3;
}
...
tagged_pids() {
  ...
  for environment in /proc/[0-9]*/environ; do
    ...
    if grep -Fzqx "BWA_R11_RUN_ROOT=$RUN_ROOT" "$environment" 2>/dev/null && \
       grep -Fzqx "BWA_R11_CANDIDATE=$candidate" "$environment" 2>/dev/null; then
      printf '%s\n' "$pid"
    fi
  done
}

printf 'Exact R11 stop targets (artifacts will be preserved):\n'
...
for candidate in "${SELECTED[@]}"; do
  ...
  if [[ "$state" != PASSED && "$state" != FAILED ]]; then
    "$PYTHON" "$ROOT/scripts/before_we_act/r11_runtime.py" status \
      --run-root "$RUN_ROOT" --candidate "$candidate" --state STOPPED \
      --stage stopped --program stop_r11_4gpu_tmux.sh \
...
printf 'Stopped only selected R11 candidates; datasets, caches, logs and checkpoints remain intact.\n'
