...
from before_we_act.train_action_grounded_probe import device_batch
from before_we_act.train_privileged_oracle import evaluate_oracle, fixed_loader
...
def main():
    ...
    dataset=ActionGroundedDataset(args.cache); device=torch.device("cuda:0"); torch.cuda.set_device(device)
    ...
    for seed in BELIEF_SEEDS:
        ...
        probes=PrivilegedOracleProbeSet().to(device); probes.load_state_dict(checkpoint["probes"])
        signal_checkpoint=Path(parent["old_n1_read_only"]["representation_checkpoints"][str(seed)]["path"])
        backbones=FrozenBeliefBackbones(temporal_checkpoint=Path(parent["b0h"]["checkpoint"]),signal_checkpoint=signal_checkpoint,visual_mean=dataset.visual_mean,visual_std=dataset.visual_std).to(device)
        ...
        if seed==BELIEF_SEEDS[0]: first_backbones=backbones
        else: del backbones
    ...
    every_seed=all(evaluation[str(seed)][name]["absolute_h_minus_oracle"]>0 for seed in BELIEF_SEEDS for name in ("validation","test"))
    task_medians={}; positive_tasks=0
    for task in SIX_TASKS:
        val=float(np.median([evaluation[str(seed)]["validation"]["per_task"][task]["relative_improvement"] for seed in BELIEF_SEEDS]))
        test=float(np.median([evaluation[str(seed)]["test"]["per_task"][task]["relative_improvement"] for seed in BELIEF_SEEDS]))
    ...
    controls=all(evaluation[str(seed)][name]["macro"]["h_oracle"]<evaluation[str(seed)][name]["macro"][control] for seed in BELIEF_SEEDS for name in ("validation","test") for control in ("h_oracle_shuffle","h_matched_capacity"))
