...
ROOT="${BWA_TEAM_BELIEF_REPO_ROOT:-/workspace/fe-pc-wam}"
RUN_ROOT="${BWA_TEAM_BELIEF_RUN_ROOT:-/workspace/bwa_runs/b-core/n2-r3-evidence-gated-persistence-v1}"
PREVIOUS_PAIRING_ROOT="${BWA_PREVIOUS_PAIRING_RUN_ROOT:-/workspace/bwa_runs/b-core/n2-r2-action-conditioned-pairing-v2}"
FAILED_BELIEF_ROOT="${BWA_FAILED_BELIEF_RUN_ROOT:-/workspace/bwa_runs/b-core/n2-predictive-team-belief-v1}"
STABILIZED_BELIEF_ROOT="${BWA_STABILIZED_BELIEF_RUN_ROOT:-/workspace/bwa_runs/b-core/n2-r1-discrete-belief-stabilization-v2}"
TEAM_SIGNAL_CACHE="${BWA_TEAM_SIGNAL_CACHE_ROOT:-/workspace/bwa_runs/shared/p1-b-core-n1-cache-v1}"
ACTION_CACHE="${BWA_TEAM_BELIEF_ACTION_CACHE_ROOT:-/workspace/bwa_runs/shared/p1-b-core-n2-action-context-v1}"
TEMPORAL_ROOT="${BWA_TEMPORAL_RUN_ROOT:-/workspace/bwa_runs/p1-step2-b0h-v7}"
ACTION_GROUNDED_ROOT="${BWA_ACTION_GROUNDED_RUN_ROOT:-/workspace/bwa_runs/b-core/n1-r1-action-grounded-belief}"
BELIEF_DISTILLATION_ROOT="${BWA_BELIEF_DISTILLATION_RUN_ROOT:-/workspace/bwa_runs/b-core/n1-r1-owner-revision-teacher-student}"
DATA_ROOT="${BWA_DATA_ROOT:-/workspace/datasets/robofactory_multitask}"
VISUAL_CACHE="${BWA_TEMPORAL_CACHE_ROOT:-/workspace/bwa_runs/shared/p1-step2-dino-history-cache-v2}"
PYTHON="${BWA_TEAM_BELIEF_PYTHON:-/venv/robofactory-act/bin/python}"
TORCHRUN="${BWA_TEAM_BELIEF_TORCHRUN:-/venv/robofactory-act/bin/torchrun}"
...
TEMPORAL_CHECKPOINT="${TEMPORAL_ROOT}/hidden_residual/formal/checkpoint_120000.pt"
NORMALIZATION="${TEMPORAL_ROOT}/contract/normalization.pt"
SCENARIO_SPLIT="${ACTION_GROUNDED_ROOT}/contract/scenario_split.json"
...
if [[ ! -f "${ACTION_CACHE}/cache_receipt.json" ]]; then
  ...
  "${TORCHRUN}" --standalone --nproc_per_node=4 \
    scripts/before_we_act/build_team_action_context_cache.py \
    --manifests "${MANIFESTS[@]}" \
    --normalization "${NORMALIZATION}" \
    --visual-cache "${VISUAL_CACHE}" \
    --signal-cache "${TEAM_SIGNAL_CACHE}" \
...
if [[ ! -f "${CONTRACT}" ]]; then
  ...
  "${PYTHON}" scripts/before_we_act/prepare_predictive_team_belief.py \
    --roadmap "${ROADMAP}" \
    --action-grounded-contract "${ACTION_GROUNDED_ROOT}/contract/r1_contract.json" \
    --student-contract "${BELIEF_DISTILLATION_ROOT}/contract/r1_5_student_contract.json" \
    --student-conclusion "${BELIEF_DISTILLATION_ROOT}/r1_5_student/conclusion.json" \
    --student-diagnostic "${BELIEF_DISTILLATION_ROOT}/r1_5_student/validation_diagnostic.json" \
    ...
    --failed-belief-run "${FAILED_BELIEF_ROOT}" \
    --stabilized-belief-run "${STABILIZED_BELIEF_ROOT}" \
    --previous-pairing-run "${PREVIOUS_PAIRING_ROOT}" \
...
if [[ ! -f "${RUN_ROOT}/contract/f0_receipt.json" ]]; then
  ...
  CUDA_VISIBLE_DEVICES=0 "${PYTHON}" scripts/before_we_act/verify_predictive_team_belief.py f0 \
    --cache "${TEAM_SIGNAL_CACHE}" --action-context-cache "${ACTION_CACHE}" \
...
if [[ ! -f "${RUN_ROOT}/contract/f1_receipt.json" ]]; then
  ...
  CUDA_VISIBLE_DEVICES=0 "${PYTHON}" -m before_we_act.train_predictive_team_belief \
    --cache "${TEAM_SIGNAL_CACHE}" --action-context-cache "${ACTION_CACHE}" \
    --contract "${CONTRACT}" --scenario-split "${SCENARIO_SPLIT}" \
...
if [[ ! -f "${RUN_ROOT}/repair_pilot_conclusion.json" ]]; then
  ...
  CUDA_VISIBLE_DEVICES=0 "${PYTHON}" -m before_we_act.train_predictive_team_belief \
    --cache "${TEAM_SIGNAL_CACHE}" --action-context-cache "${ACTION_CACHE}" \
    --contract "${CONTRACT}" --scenario-split "${SCENARIO_SPLIT}" \
    --output "${PILOT_ROOT}" --seed 20260815 --updates 4000 --workers 2 \
    --save-every 1000 --log-every 100 --evaluate-at-end \
    >"${RUN_ROOT}/logs/repair_pilot.log" 2>&1
  "${PYTHON}" scripts/before_we_act/analyze_belief_repair_pilot.py \
...
if [[ "${BWA_TEAM_BELIEF_AUTHORIZE_FORMAL:-0}" != 1 ]]; then
  write_status PASSED repair_pilot_complete "${decision}; formal training was not started"
  printf 'TEAM_BELIEF_REPAIR_PILOT_COMPLETED status=%s formal_training_started=false\n' "${decision}"
  exit 0
fi
if [[ "${decision}" != PASSED_CAUSAL_REPAIR_GATES_FORMAL_TRAINING_REQUIRES_OWNER_DECISION ]]; then
...
for gpu in 0 1 2; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -m before_we_act.train_predictive_team_belief \
    --cache "${TEAM_SIGNAL_CACHE}" --action-context-cache "${ACTION_CACHE}" \
    --contract "${CONTRACT}" --scenario-split "${SCENARIO_SPLIT}" \
...
"${PYTHON}" scripts/before_we_act/analyze_predictive_team_belief.py \
...
PIDS=()
run_model_validation b0h b0h "${TEMPORAL_CHECKPOINT}" 3 & PIDS+=("$!")
for gpu in 0 1 2; do
  seed="${SEEDS[$gpu]}"
...
if [[ "${BWA_TEAM_BELIEF_AUTHORIZE_VALIDATION_DIAGNOSTIC:-0}" == 1 && "${final_status}" == POSITIVE_SIGNAL ]]; then
  write_status RUNNING validation20 "running the pre-selected N2 seed on the frozen W10/B0-H Validation20 protocol"
  bash scripts/before_we_act/run_team_belief_validation.sh \
...
elif [[ "${BWA_TEAM_BELIEF_AUTHORIZE_VALIDATION_DIAGNOSTIC:-0}" == 1 ]]; then
...
trap - ERR
printf 'TEAM_BELIEF_PIPELINE_COMPLETED status=%s\n' "${final_status}"
