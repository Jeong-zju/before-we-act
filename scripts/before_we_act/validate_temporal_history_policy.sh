...
RUN_ROOT="${TEMPORAL_RUN_ROOT:?TEMPORAL_RUN_ROOT is required}"
CHECKPOINT="${TEMPORAL_CHECKPOINT:?TEMPORAL_CHECKPOINT is required}"
EPISODES="${TEMPORAL_VALIDATION_EPISODES:?TEMPORAL_VALIDATION_EPISODES is required}"
SEED_ROOT="${TEMPORAL_SEED_ROOT:-/workspace/bwa_runs/w10-six-task-v1/seeds/validation}"
PYTHON_BIN="${TEMPORAL_PYTHON:-/venv/robofactory-act/bin/python}"
ROBOFACTORY_ROOT="${TEMPORAL_ROBOFACTORY_ROOT:-/workspace/RoboFactory}"
EVAL_ROOT="${RUN_ROOT}/evaluation/validation${EPISODES}"
LOG_ROOT="${RUN_ROOT}/logs"
STATUS="${RUN_ROOT}/validation${EPISODES}_status.json"
EVALUATOR="${ROOT}/before_we_act/evaluate_temporal_history_policy.py"
