...
from before_we_act.raw_team_signal_data import BalancedTeamBatchSampler, RawTeamSignalDataset, TeamSampleRequest
from before_we_act.raw_team_signal_model import RawTeamSignalEncoder, representation_losses
from before_we_act.train_raw_team_signal import atomic_json, device_batch
...
def main() -> None:
    ...
    for task, episode_index in by_task.items():
        ...
        for t in (0, episode.length // 2, episode.length - 1):
            row = dataset[TeamSampleRequest(episode_index, 0, t, f"f0:{task}:{t}", task)]
            audit.append(
                {
                    "task": task,
                    "time": t,
    ...
    boundary_ok = (
        not (RawTeamSignalDataset.RUNTIME_FIELDS & RawTeamSignalDataset.TEACHER_TARGET_FIELDS)
        and not (RawTeamSignalDataset.RUNTIME_FIELDS & RawTeamSignalDataset.AUDIT_ONLY_FIELDS)
        and all(row["runtime_shapes_ok"] for row in audit)
    )
    sampler = BalancedTeamBatchSampler(dataset.episodes, updates=2, data_seed=20260815)
    ...
    model = RawTeamSignalEncoder(dropout=0.0).to(device)
    ...
    resumed = RawTeamSignalEncoder(dropout=0.0).to(device); resumed.load_state_dict(saved_model)
    ...
    receipt = {
        ...
        "f0": {"boundary_ok": boundary_ok, "samples": audit, "runtime_fields": sorted(RawTeamSignalDataset.RUNTIME_FIELDS), "teacher_target_fields": sorted(RawTeamSignalDataset.TEACHER_TARGET_FIELDS), "audit_only_fields": sorted(RawTeamSignalDataset.AUDIT_ONLY_FIELDS)},
