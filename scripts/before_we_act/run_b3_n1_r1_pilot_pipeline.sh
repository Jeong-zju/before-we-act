...
if [[ ! -f "${RUN_ROOT}/contract/r1_3_pilot_contract.json" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1_r1_pilot.py \
    --dataset-root /workspace/datasets/robofactory_multitask \
    --r1-contract "${RUN_ROOT}/contract/r1_contract.json" \
    --collector "${REPO_ROOT}/scripts/before_we_act/run_b3_n1_r1_pilot.py" \
...
if [[ ! -f "${RUN_ROOT}/r1_3_counterfactual_pilot/conclusion.json" ]]; then
  "${PYTHON}" scripts/before_we_act/run_b3_n1_r1_pilot.py \
