...
if [[ -f "$CACHE" ]]; then
  ...
  PREPARE_CMD="exec env CUDA_VISIBLE_DEVICES=0 BWA_R13_RUN_ROOT='$RUN_ROOT' BWA_R13_CANDIDATE=prepare PYTHONPATH='$ROOT' '$PYTHON' '$ROOT/scripts/before_we_act/prepare_r13_world_cache.py' --index '$FULL_INDEX' --belief-config '$ROOT/configs/before_we_act/r11_belief/p0.yaml' --belief-checkpoint '$BELIEF_CHECKPOINT' --belief-sha256 '$BELIEF_SHA' --action-config '$ROOT/configs/before_we_act/r12_action/e1_p2.yaml' --action-checkpoint '$ACTION_CHECKPOINT' --action-sha256 '$ACTION_SHA' --output '$CACHE' --heartbeat '$RUN_ROOT/shared/cache_heartbeat.json' --device cuda:0 2>&1 | tee -a '$PREPARE_LOG'; code=\${PIPESTATUS[0]}; if ((code == 0)); then exec '$PYTHON' '$ROOT/scripts/before_we_act/verify_r13_world_cache.py' --cache '$CACHE' --belief-sha256 '$BELIEF_SHA' --action-sha256 '$ACTION_SHA' --output '$RUN_ROOT/shared/cache.json'; else exit \$code; fi"
