...
def sha(path):
 ...
 return h.hexdigest()
d={"schema_version":1,"protocol":"r15_stack_original_plus_raw_success_expert_source_aware_v2","expert_index":os.path.realpath(index),"expert_index_sha256":sha(index),"reference_run_root":os.path.realpath(reference),"updates":int(updates),"batch_size":int(batch),"expert_rows":int(expert_rows),"learning_rate":float(learning_rate),"warmup":int(warmup),"created_at":datetime.datetime.now(datetime.timezone.utc).isoformat().replace('+00:00','Z')}
