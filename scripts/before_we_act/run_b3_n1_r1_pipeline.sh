...
RUN_ROOT="${BWA_R1_RUN_ROOT:-/workspace/bwa_runs/b-core/n1-r1-action-grounded-belief}"
N1_ROOT="${BWA_N1_RUN_ROOT:-/workspace/bwa_runs/p1-b-core-n1-v1}"
N1_CACHE="${BWA_N1_CACHE_ROOT:-/workspace/bwa_runs/shared/p1-b-core-n1-cache-v1}"
STEP2_ROOT="${BWA_STEP2_ROOT:-/workspace/bwa_runs/p1-step2-b0h-v7}"
...
if [[ ! -f "${RUN_ROOT}/contract/r1_0_receipt.json" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1_r1.py \
    --n1-cache "${N1_CACHE}" --n1-run "${N1_ROOT}" \
    --step2-run "${STEP2_ROOT}" --output "${RUN_ROOT}" \
    >"${RUN_ROOT}/logs/r1_0_prepare.log" 2>&1
fi

if [[ ! -f "${RUN_ROOT}/contract/f0_f1_receipt.json" ]]; then
  CUDA_VISIBLE_DEVICES=0 "${PYTHON}" scripts/before_we_act/verify_b3_n1_r1.py \
    --cache "${N1_CACHE}" --contract "${RUN_ROOT}/contract/r1_contract.json" \
...
for gpu in 0 1 2; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -m before_we_act.train_b3_n1_r1_fair_probe \
    --cache "${N1_CACHE}" --contract "${RUN_ROOT}/contract/r1_contract.json" \
    --scenario-split "${RUN_ROOT}/contract/scenario_split.json" \
...
CUDA_VISIBLE_DEVICES=3 "${PYTHON}" scripts/before_we_act/analyze_b3_n1_r1_fair.py \
  --cache "${N1_CACHE}" --contract "${RUN_ROOT}/contract/r1_contract.json" \
  --scenario-split "${RUN_ROOT}/contract/scenario_split.json" \
