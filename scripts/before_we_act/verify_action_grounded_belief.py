...
def main() -> None:
    ...
    signal_checkpoint = contract["old_n1_read_only"]["representation_checkpoints"][str(seed)]
    ...
    checks = {
        ...
        "n1_hash_matches": sha256_file(Path(signal_checkpoint["path"])) == signal_checkpoint["sha256"],
    ...
    resumed = ActionGroundedProbeSet().to(device)
    ...
    payload = {
        ...
        "input_boundary": {
            "runtime_fields": sorted(RawTeamSignalDataset.RUNTIME_FIELDS),
            "audit_fields_not_forwarded": sorted(RawTeamSignalDataset.AUDIT_ONLY_FIELDS),
            "teacher_targets_not_forwarded": sorted(RawTeamSignalDataset.TEACHER_TARGET_FIELDS),
        },
    }
