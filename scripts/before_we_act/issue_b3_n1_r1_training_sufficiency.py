...
def main() -> None:
    ...
    index: dict[str, dict] = {}
    expected_updates = list(range(R1_EVAL_EVERY, R1_MAX_UPDATES + 1, R1_EVAL_EVERY))
    for seed in R1_SEEDS:
        ...
        for condition in R1_CONDITIONS:
            ...
            platform = (
                updates[-1] >= R1_EARLIEST_PLATFORM
        ...
        receipt = {
            ...
            "minimum_exposure_met": int(status["update"]) >= R1_MIN_UPDATES,
            "earliest_platform": R1_EARLIEST_PLATFORM,
            "maximum_updates": R1_MAX_UPDATES,
            "maximum_updates_reached": int(status["update"]) == R1_MAX_UPDATES,
            ...
            "all_trainable_conditions_platform": all(
                conditions[name]["training_sufficient"] for name in R1_CONDITIONS
            ),
