...
REPO_ROOT="${BWA_REPO_ROOT:-/workspace/fe-pc-wam-b-core}"
BASE_RUN_ROOT="${BWA_R1_BASE_RUN_ROOT:-/workspace/bwa_runs/b-core/n1-r1-action-grounded-belief}"
RUN_ROOT="${BWA_R1_REVISION_RUN_ROOT:-/workspace/bwa_runs/b-core/n1-r1-owner-revision-teacher-student}"
...
if [[ ! -f "${OWNER_REVISION}" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1_r1_owner_revision.py \
...
if [[ ! -f "${TEACHER_CONTRACT}" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1_r1_teacher.py \
    --parent-contract "${PARENT_CONTRACT}" \
    --fair-conclusion "${FAIR_CONCLUSION}" \
...
if [[ ! -f "${RUN_ROOT}/contract/f0_receipt.json" ]]; then
  "${PYTHON}" scripts/before_we_act/verify_b3_n1_r1_owner_revision.py \
    --owner-revision "${OWNER_REVISION}" \
...
for gpu in 0 1 2; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -m before_we_act.train_b3_n1_r1_teacher \
    --cache "${N1_CACHE}" \
    --parent-contract "${PARENT_CONTRACT}" \
    --teacher-contract "${TEACHER_CONTRACT}" \
...
if [[ ! -f "${RUN_ROOT}/r1_4_teacher/conclusion.json" ]]; then
  CUDA_VISIBLE_DEVICES=3 "${PYTHON}" scripts/before_we_act/analyze_b3_n1_r1_teacher.py \
    --cache "${N1_CACHE}" \
    --parent-contract "${PARENT_CONTRACT}" \
    --teacher-contract "${TEACHER_CONTRACT}" \
    --scenario-split "${SCENARIO_SPLIT}" \
...
if [[ ! -f "${STUDENT_CONTINUATION}" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1_r1_student_continuation.py \
    --owner-revision "${OWNER_REVISION}" \
...
if [[ ! -f "${STUDENT_CONTRACT}" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1_r1_student.py \
    --parent-contract "${PARENT_CONTRACT}" \
    --teacher-contract "${TEACHER_CONTRACT}" \
    --teacher-conclusion "${RUN_ROOT}/r1_4_teacher/conclusion.json" \
    --fair-conclusion "${FAIR_CONCLUSION}" \
...
for gpu in 0 1 2; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -m before_we_act.train_b3_n1_r1_student \
    --cache "${N1_CACHE}" \
    --parent-contract "${PARENT_CONTRACT}" \
...
if [[ ! -f "${RUN_ROOT}/r1_5_student/conclusion.json" ]]; then
  CUDA_VISIBLE_DEVICES=3 "${PYTHON}" scripts/before_we_act/analyze_b3_n1_r1_student.py \
    --cache "${N1_CACHE}" \
    --parent-contract "${PARENT_CONTRACT}" \
    --student-contract "${STUDENT_CONTRACT}" \
    --scenario-split "${SCENARIO_SPLIT}" \
