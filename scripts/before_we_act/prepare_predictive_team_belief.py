...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--failed-belief-run", type=Path, required=True)
    parser.add_argument("--stabilized-belief-run", type=Path, required=True)
    parser.add_argument("--previous-pairing-run", type=Path, required=True)
...
def main() -> None:
    ...
    failed_status_path = args.failed_belief_run / "pipeline_status.json"
    failed_contract_path = args.failed_belief_run / "contract/n2_contract.json"
    ...
    stabilized_conclusion_path = args.stabilized_belief_run / "repair_pilot_conclusion.json"
    stabilized_pilot_path = (
        args.stabilized_belief_run
        / "repair_pilot"
        / "seed_20260815"
        / "status.json"
    )
    ...
    previous_pairing_conclusion_path = args.previous_pairing_run / "repair_pilot_conclusion.json"
    previous_pairing_status_path = (
        args.previous_pairing_run / "repair_pilot/seed_20260815/status.json"
    )
    previous_pairing = json.loads(previous_pairing_conclusion_path.read_text(encoding="utf-8"))
    previous_pairing_status = json.loads(previous_pairing_status_path.read_text(encoding="utf-8"))
    if previous_pairing.get("status") != "FAILED_ACTION_CONDITIONED_FUTURE":
        raise RuntimeError("the previous R2 future failure evidence changed")
    previous_pairing_validation = previous_pairing_status["selected_validation"]
    failed_seed_evidence = {}
    for seed in SEEDS:
        evaluation_path = args.failed_belief_run / f"training/seed_{seed}/evaluations.jsonl"
        rows = [
            json.loads(line)
    ...
    payload = {
        ...
        "inputs": {
            ...
            "n1_metadata": str((args.signal_cache / "metadata.json").resolve()),
            ...
            "superseded_gaussian_n2_run": str(args.failed_belief_run.resolve()),
            ...
            "previous_r2_conclusion": str(previous_pairing_conclusion_path.resolve()),
            "previous_r2_conclusion_sha256": sha256_file(
                previous_pairing_conclusion_path
            ),
            "previous_r2_pilot_status": str(previous_pairing_status_path.resolve()),
        ...
        "previous_r2_failure_evidence": {
            "status": previous_pairing["status"],
            "future_observable_mse": previous_pairing_validation[
                "future_observable_mse"
            ],
            "uncertainty_occlusion": previous_pairing_validation[
