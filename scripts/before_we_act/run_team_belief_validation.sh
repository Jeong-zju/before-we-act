...
W10_SUMMARY="${BWA_TEAM_BELIEF_REFERENCE_SUMMARY:-/workspace/bwa_runs/w10-six-task-v1/evaluation/validation/summary.json}"
TEMPORAL_SUMMARY="${BWA_TEAM_BELIEF_TEMPORAL_SUMMARY:-/workspace/bwa_runs/p1-step2-b0h-v7/hidden_residual/evaluation/validation20/summary.json}"
PYTHON="${BWA_TEAM_BELIEF_PYTHON:-/venv/robofactory-act/bin/python}"
ROBOFACTORY_ROOT="${BWA_ROBOFACTORY_ROOT:-/workspace/RoboFactory}"
CONCLUSION="${BWA_TEAM_BELIEF_CONCLUSION:-${RUN_ROOT}/conclusion.json}"
...
[[ -f "${W10_SUMMARY}" && -f "${TEMPORAL_SUMMARY}" ]] || \
...
"${PYTHON}" scripts/before_we_act/summarize_team_belief_validation.py \
  --conclusion "${CONCLUSION}" --validation-root "${OUTPUT_ROOT}" \
  --seed-root "${SEED_ROOT}" --w10-summary "${W10_SUMMARY}" \
  --temporal-summary "${TEMPORAL_SUMMARY}" --output "${SUMMARY}"
printf 'TEAM_BELIEF_VALIDATION_COMPLETED summary=%s\n' "${SUMMARY}"
