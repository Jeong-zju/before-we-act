...
[[ "$RUN_ID" =~ ^[A-Za-z0-9_.-]+$ && "$CANDIDATE" =~ ^p[1-3]$ && "$GPU_INDEX" =~ ^[0-3]$ && "$MODE" =~ ^(act_temporal_ensemble|mild_temporal_ensemble|balanced_temporal_ensemble|recent_temporal_ensemble|responsive_temporal_ensemble|latest_chunk)$ ]] || { printf 'valid run/candidate/GPU/mode required\n' >&2; exit 2; }
...
case "$MODE" in mild_temporal_ensemble) LABEL=w12_mild_decay_0p02 ;; balanced_temporal_ensemble) LABEL=w12_balanced_decay_0p05 ;; recent_temporal_ensemble) LABEL=w12_recent_decay_0p10 ;; responsive_temporal_ensemble) LABEL=w12_responsive_decay_0p20 ;; latest_chunk) LABEL=w12_latest_chunk ;; *) LABEL=checkpoint_act_temporal_ensemble ;; esac
