...
[[ "$RUN_ID" =~ ^[A-Za-z0-9_.-]+$ && "$CANDIDATE" =~ ^p[1-3]$ && "$GPU_INDEX" =~ ^[0-3]$ && "$MODE" =~ ^(act_temporal_ensemble|balanced_temporal_ensemble|recent_temporal_ensemble|latest_chunk)$ ]] || { printf 'valid run/candidate/GPU/mode required\n' >&2; exit 2; }
...
case "$MODE" in balanced_temporal_ensemble) LABEL=w12_balanced_decay_0p05 ;; recent_temporal_ensemble) LABEL=w12_recent_decay_0p10 ;; latest_chunk) LABEL=w12_latest_chunk ;; *) LABEL=checkpoint_act_temporal_ensemble ;; esac
