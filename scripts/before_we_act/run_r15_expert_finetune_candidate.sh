...
[[ "$CURRENT_BRANCH" =~ ^bwa/r15-(closed-loop-evolution|expert-evolution|phase-balanced-expert|role-query-specialist)$ && "${IDENTITY[0]}" == "$ROOT" && "${IDENTITY[1]}" == "$CURRENT_BRANCH" && "${IDENTITY[2]}" == "$(git -C "$ROOT" rev-parse HEAD)" && "${IDENTITY[3]}" == "$CHECKPOINT" && -z "$(git -C "$ROOT" status --porcelain)" ]] || { printf 'R15 expert run identity differs\n' >&2; exit 3; }
