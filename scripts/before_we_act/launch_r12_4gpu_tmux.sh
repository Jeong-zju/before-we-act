...
BELIEF_SHA256=a453f3d0e7248e36c99d3df716705224117ef1560a68974e12272828f311c9df
...
WORKTREE_ROOT=/workspace/bwa_worktrees/r12
...
BRANCHES=(bwa/r12-p0-openpi-action-expert-component bwa/r12-p1-smolvla-action-expert-component bwa/r12-p2-rdt-dit-component bwa/r12-p3-consistency-policy-component)
...
if [[ ! -f "$ACTION_CACHE" ]]; then
  if tmux has-session -t bwa-r12-prepare 2>/dev/null; then
    printf 'reusing active shared cache preparation session: bwa-r12-prepare\n'
  else
    tmux new-session -d -s bwa-r12-prepare -n cache \
      "cd '$FE_ROOT' && exec env PYTHONPATH='$FE_ROOT' '$PYTHON' '$FE_ROOT/scripts/before_we_act/prepare_r12_action_cache.py' --r11-cache '$R11_CACHE' --parent-checkpoint '$NORMALIZATION_CHECKPOINT' --data-root '$DATA_ROOT' --output '$ACTION_CACHE' --state '/workspace/bwa_runs/shared/r12_action_cache_state.json' --heartbeat '/workspace/bwa_runs/shared/r12_action_cache_heartbeat.json'"
  fi
fi
...
for candidate in "${SELECTED[@]}"; do
  index="${candidate#p}"
  worktree="$WORKTREE_ROOT/$candidate"
  tmux new-session -d -s "bwa-r12-$candidate" -n pipeline \
