...
def init(args):
    ...
    atomic_json(
        manifest,
        {
            ...
            "tmux_sessions": {candidate: f"bwa-r12-{candidate}" for candidate in CANDIDATES},
            "shared_data": "/workspace/datasets/robofactory_multitask",
            "shared_hf_cache": "/workspace/.cache/huggingface",
            "shared_action_cache": "/workspace/bwa_runs/shared/r12_action_cache.pt",
            ...
            "acceptance_rules": [
                ...
                "formal 2000 updates complete; offline control-cycle smoke is finite",
...
def render(run_root: Path, selected):
    ...
    for candidate in selected:
        ...
        output.extend([
            ...
            f"  entered_acceptance={status.get('stage') in ('acceptance','complete')} acceptance={acceptance.get('status', 'PENDING')} progress={len(acceptance.get('acceptance', []))}/10 failed={[row['id'] for row in acceptance.get('acceptance', []) if not row.get('passed')]}",
