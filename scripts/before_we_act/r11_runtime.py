#!/usr/bin/env python3
"""Atomic status, real heartbeat and unified four-candidate R11 monitor."""
...
def init(args):
    ...
    atomic_json(
        manifest,
        {
            ...
            "tmux_sessions": {candidate: f"bwa-r11-{candidate}" for candidate in CANDIDATES},
            ...
            "acceptance_rules": [
                "official source commit and license are pinned and preserved",
                "minimal copied component passes upstream parity and patch audit",
                "no full upstream runtime dependency or forbidden input leakage",
                "two-update train/save/strict-restore and formal 10000 updates complete",
                "W10 action hashes are exactly equal; Gate20 is N/A",
                "no representation quality threshold; highest frozen screen score wins among valid candidates",
            ],
        },
    )
...
def acceptance_result(root: Path):
    """Return only the authoritative R11 acceptance receipt."""
...
def render(run_root: Path, selected):
    ...
    gpus = gpu_rows()
    output = [f"BWA R11 monitor | run={manifest.get('run_id', run_root.name)} | {now()}", f"root={run_root}"]
