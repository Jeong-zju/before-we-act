...
RUN_ROOT=/workspace/bwa_runs/r12r4-current
SELECTION=all
GRACE=30
DRY_RUN=0
...
[[ "$(jq -r '.round' "$MANIFEST")" == R12-R4 && "$(realpath -m "$(jq -r '.run_root' "$MANIFEST")")" == "$(realpath -m "$RUN_ROOT")" ]] || { printf 'R12-R4 run manifest identity mismatch\n' >&2; exit 3; }
...
printf 'Exact R12-R4 stop targets (all caches/logs/checkpoints are preserved):\n'
...
if ((DRY_RUN)); then
  if ((${#SELECTED[@]} == 4)); then
    printf '  shared cache sessions considered only for all-stop: bwa-r12r4-cache-rank0..3 and bwa-r12r4-cache-index\n'
  fi
...
AUXILIARY_SESSIONS=(bwa-r12r4-cache-rank0 bwa-r12r4-cache-rank1 bwa-r12r4-cache-rank2 bwa-r12r4-cache-rank3 bwa-r12r4-cache-index)
if ((${#SELECTED[@]} == 4)); then
...
for signal in TERM KILL; do
  ...
  for candidate in "${SELECTED[@]}"; do
    mapfile -t pids < <(tagged_pids "$candidate")
    if ((${#pids[@]})); then
      found=1; printf 'sending SIG%s only to %s tagged PIDs: %s\n' "$signal" "$candidate" "${pids[*]}"
...
for candidate in "${SELECTED[@]}"; do
  ...
  if [[ "$state" != PASSED && "$state" != FAILED ]]; then
    "$PYTHON" "$ROOT/scripts/before_we_act/r12_runtime.py" status --run-root "$RUN_ROOT" --candidate "$candidate" --state STOPPED --stage stopped --program stop_r12_r4_4gpu_tmux.sh --detail "graceful stop requested; latest checkpoint and all artifacts preserved" --pid 0 --child-pid 0 --exit-code 130 --log "$RUN_ROOT/candidates/$candidate/logs/candidate.log"
  fi
done
if ((${#SELECTED[@]} == 4)); then
  for session in "${AUXILIARY_SESSIONS[@]}"; do
    if tmux has-session -t "$session" 2>/dev/null; then
      printf 'closing exact shared R12-R4 cache session: %s\n' "$session"
...
printf 'Stopped only selected R12-R4 targets; datasets, HF cache, feature cache, logs and checkpoints remain intact.\n'
