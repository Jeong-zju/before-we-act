...
def build_split(cache: Path) -> dict:
    metadata, episodes = load_team_signal_metadata(cache)
    result: list[dict] = []
    ...
    for task in SIX_TASKS:
        ...
        ordered_groups = sorted(
            ...
            key=lambda group: hashlib.sha256(
                (
                    f"{BELIEF_DATA_SEED}:{task}:group:{group}:"
                    + ":".join(
...
def selected_signal_checkpoints(signal_run: Path) -> dict[str, dict]:
    result: dict[str, dict] = {}
    for seed in BELIEF_SEEDS:
        root = signal_run / "representation" / f"seed_{seed}"
...
def main() -> None:
    ...
    old_signal_conclusion = args.signal_run / "n1_conclusion.json"
    old_signal_contract = args.signal_run / "contract" / "n1_contract.json"
    temporal_checkpoint = args.temporal_run / "hidden_residual" / "formal" / "checkpoint_120000.pt"
    temporal_contract = args.temporal_run / "contract" / "step2_contract.json"
    for path in (old_signal_conclusion, old_signal_contract, temporal_checkpoint, temporal_contract):
        if not path.is_file():
            raise FileNotFoundError(path)
    old_status = json.loads(old_signal_conclusion.read_text(encoding="utf-8")).get("status")
    if old_status != "INCONCLUSIVE_TRAINING_NOT_CONVERGED":
        raise RuntimeError(f"old N1 conclusion differs: {old_status}")

    split = build_split(args.signal_cache)
    atomic_json(split_path, split)
    signal_checkpoints = selected_signal_checkpoints(args.signal_run)
    contract: dict[str, object] = {
        ...
        "old_n1_read_only": {
            "root": str(args.signal_run.resolve()),
            "contract_sha256": sha256_file(old_signal_contract),
            "conclusion_sha256": sha256_file(old_signal_conclusion),
            "status": old_status,
            "representation_checkpoints": signal_checkpoints,
        },
        "b0h": {
            "checkpoint": str(temporal_checkpoint.resolve()),
            "checkpoint_sha256": sha256_file(temporal_checkpoint),
            "step2_contract": str(temporal_contract.resolve()),
            "step2_contract_sha256": sha256_file(temporal_contract),
            "frozen": True,
            "hidden": "TemporalHistoryPolicy._encode_history history_summary from its trained hidden-residual checkpoint",
        ...
        "r1_1": {
            ...
            "data_seed": BELIEF_DATA_SEED,
            "old_n1_capacity": 16,
            "conditions": list(ACTION_GROUNDED_CONDITIONS),
        ...
        "r1_4_r1_5": {
            ...
            "maximum_updates_each_trainable_group": BELIEF_MAX_UPDATES,
