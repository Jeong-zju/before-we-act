...
[[ "$(jq -r '.round' "$MANIFEST")" == R10 && "$(realpath -m "$(jq -r '.run_root' "$MANIFEST")")" == "$(realpath -m "$RUN_ROOT")" ]] || {
...
tagged_pids() {
  ...
  for environment in /proc/[0-9]*/environ; do
    ...
    if grep -Fzqx "BWA_R10_RUN_ROOT=$RUN_ROOT" "$environment" 2>/dev/null && \
       grep -Fzqx "BWA_R10_CANDIDATE=$candidate" "$environment" 2>/dev/null; then
      printf '%s\n' "$pid"
    fi
  done
}

printf 'Exact R10 stop targets (artifacts will be preserved):\n'
...
for candidate in "${SELECTED[@]}"; do
  ...
  if [[ "$state" != PASSED && "$state" != FAILED ]]; then
    "$PYTHON" "$ROOT/scripts/before_we_act/r10_runtime.py" status \
      --run-root "$RUN_ROOT" --candidate "$candidate" --state STOPPED \
      --stage stopped --program stop_r10_4gpu_tmux.sh \
...
printf 'Stopped only selected R10 candidates; datasets, caches, logs and checkpoints remain intact.\n'
