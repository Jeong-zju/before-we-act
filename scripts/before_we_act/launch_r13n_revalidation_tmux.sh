...
command=("$FE_ROOT/scripts/before_we_act/run_r13n_revalidation.sh" --run-root "$RUN_ROOT" --source-run-root "$SOURCE_RUN_ROOT" --session "$SESSION")
printf -v command_text '%q ' "${command[@]}"
printf 'branch=%s commit=%s session=%s run_root=%s source_run_root=%s\n' "$(git -C "$FE_ROOT" branch --show-current)" "$(git -C "$FE_ROOT" rev-parse HEAD)" "$SESSION" "$RUN_ROOT" "$SOURCE_RUN_ROOT"
printf 'command=%s\n' "$command_text"
((DRY_RUN)) && exit 0
