...
RUN_ROOT="${BWA_N1_RUN_ROOT:-/workspace/bwa_runs/p1-b-core-n1-v1}"
CACHE_ROOT="${BWA_N1_CACHE_ROOT:-/workspace/bwa_runs/shared/p1-b-core-n1-cache-v1}"
...
if [[ ! -f "${RUN_ROOT}/contract/cache_receipt.json" ]]; then
  "${PYTHON}" scripts/before_we_act/prepare_b3_n1.py \
    --manifests "${manifests[@]}" \
...
if [[ ! -f "${RUN_ROOT}/contract/f0_f1_receipt.json" ]]; then
  CUDA_VISIBLE_DEVICES=0 "${PYTHON}" scripts/before_we_act/verify_b3_n1.py \
...
for gpu in 0 1 2; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -m before_we_act.train_b3_n1_representation \
    --cache "${CACHE_ROOT}" --contract "${RUN_ROOT}/contract/n1_contract.json" \
...
for gpu in 0 1 2; do
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -m before_we_act.train_b3_n1_probe \
    --cache "${CACHE_ROOT}" --contract "${RUN_ROOT}/contract/n1_contract.json" \
...
"${PYTHON}" scripts/before_we_act/analyze_b3_n1.py \
