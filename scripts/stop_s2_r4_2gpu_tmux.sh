...
if [[ -z "${RUN_ROOT}" ]]; then
  RUN_ROOT="${FE_ROOT}/outputs/s2_r4_runs/${RUN_ID}"
fi
RUN_ROOT="$(realpath -m "${RUN_ROOT}")"
MANIFEST="${RUN_ROOT}/run_manifest.json"
test -f "${MANIFEST}" || {
  printf >&2 'Missing S2-R4 run manifest: %s\n' "${MANIFEST}"
  exit 3
}
...
if [[ "${MANIFEST_RUN_ID}" != "${RUN_ID}" || "${ROUND_ID}" != "s2-r4" ]]; then
  printf >&2 'Run manifest does not match S2-R4 run %s.\n' "${RUN_ID}"
  exit 3
fi
TARGET_WINDOWS=(
...
for name in "${TARGET_WINDOWS[@]}"; do
  if [[ "${CURRENT_WINDOW}" == "${name}" ]]; then
    printf >&2 'Run stop from a non-S2-R4 base window; current=%s.\n' \
      "${CURRENT_WINDOW}"
    exit 3
  fi
done
...
run_process_ids() {
  ...
  for environment in /proc/[0-9]*/environ; do
    ...
    if grep -Fzqx "S2_R4_RUN_ROOT=${RUN_ROOT}" "${environment}" 2>/dev/null; then
      printf '%s\n' "${pid}"
    fi
  done
}

printf 'S2-R4 stop target: run=%s root=%s tmux=%s\n' \
...
mapfile -t remaining < <(run_process_ids)
if (( ${#remaining[@]} > 0 )); then
  printf >&2 'Failed to terminate S2-R4 processes: %s\n' "${remaining[*]}"
  exit 5
fi
printf 'S2-R4 stopped. Permanent tmux, shared data, artifacts, checkpoints, resumes, logs and evaluations are preserved.\n'
