#!/usr/bin/env python3
"""Create, heartbeat, monitor, and summarize one two-GPU S2-R4 run."""
...
FORMAT_VERSION = "wam.robofactory.s2_r4.runtime/1"
CANDIDATES = ("P0", "P1")
...
def render_monitor(run_root: Path) -> str:
    ...
    lines = [
        (
            f"WAM S2-R4 monitor | run={manifest.get('run_id', run_root.name)} | "
...
def _acceptance_lines(value: Mapping[str, Any]) -> list[str]:
    if not value:
        return [
            "S2-R4 special acceptance: pending (wait for both five-task evaluations)"
        ]
    decision = (
        "PASS -> enter S3"
        if value.get("passed")
        else "FAIL -> keep R3 local and stop cross-agent path"
    )
    lines = [f"S2-R4 special acceptance: {decision}"]
    ...
    if isinstance(per_task, Mapping):
        lines.append(
            "task                         P0 own    P1 own    P0-P1     "
            "peer/shared  persistence peer Δ      CI95 lower"
        )
        for task_id, row in sorted(per_task.items()):
            ...
            lines.append(
                f"{task_id:<28}"
                f"{float(row['p0_normal_composite_future_loss']):<10.5g}"
                f"{float(row['p1_own_normal_composite_future_loss']):<10.5g}"
                f"{float(row['p0_minus_p1']):<10.5g}"
                f"{float(row['p1_peer_shared_composite_future_loss']):<13.5g}"
                f"{float(row['persistence_composite_future_loss']):<12.5g}"
                f"{float(row['p1_peer_action_shuffle_delta']):<12.5g}"
                f"{float(row['p1_peer_action_shuffle_bootstrap_95_lower']):.5g}"
            )
    return lines
