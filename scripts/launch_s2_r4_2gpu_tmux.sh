...
WORKTREE_ROOT="${S2_R4_WORKTREE_ROOT:-${WORKSPACE_ROOT}/worktrees}"
RUN_ID="${S2_R4_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${S2_R4_RUN_ROOT:-${FE_ROOT}/outputs/s2_r4_runs/${RUN_ID}}"
FOCUS_MONITOR=auto
DRY_RUN=0
HF_TOKEN_INPUT=""
...
CONFIG_REL="configs/wam_flow/s2_r4_future.yaml"
SESSION="${S2_R4_TMUX_SESSION:-<current-or-only-existing-session>}"
WINDOW_PREFIX="${S2_R4_WINDOW_PREFIX:-${RUN_ID}}"
...
resolve_existing_tmux_session() {
  local requested="${S2_R4_TMUX_SESSION:-}"
  local sessions=()
  if [[ -n "${TMUX:-}" ]]; then
...
ensure_uv() {
  ...
  local bootstrap="${WORKSPACE_ROOT}/.s2-r4-tools/uv"
...
fetch_candidate_branches() {
  ...
  for branch in "${BRANCHES[@]}"; do
    git -C "${FE_ROOT}" show-ref \
      ...
      printf >&2 'Missing remote S2-R4 branch: origin/%s\n' "${branch}"
      exit 3
    }
...
candidate_command() {
  ...
  shell_join \
    ...
    "GPU_INDEX=${gpu}" \
    "S2_R4_RUN_ID=${RUN_ID}" \
    "S2_R4_RUN_ROOT=${RUN_ROOT}" \
    "S2_R4_READY_FILE=${READY_FILE}" \
    "S2_R4_FAILED_FILE=${FAILED_FILE}" \
    "S2_R4_BASE_REPO=${FE_ROOT}" \
    "S2_R4_UV_CACHE_DIR=${SHARED_UV_CACHE}" \
    "S2_R4_UV_ENV=${SHARED_UV_ENV}" \
    bash scripts/run_s2_r4_candidate.sh
}

print_plan() {
  printf 'S2-R4 parent: %s @ %s\n' \
  ...
  for index in "${!CANDIDATES[@]}"; do
    if (( index == 0 )); then
      candidate_path="${FE_ROOT}"
    else
      candidate_path="${WORKTREE_ROOT}/${WORKTREE_NAMES[index]}"
    fi
    printf '%s GPU%s  %-35s worktree=%s/%s window=%s\n' \
      "${CANDIDATES[index]}" "${index}" "${BRANCHES[index]}" \
      "$(dirname "${candidate_path}")" "$(basename "${candidate_path}")" \
...
if (( DRY_RUN )); then
  ...
  printf 'R3-W1: reuse verified checkpoint or rebuild 10k on GPU0 with persistent resume.\n'
  printf 'Monitor: current program, status, heartbeat, progress, GPU, and every R4 gate.\n'
  exit 0
fi
...
if [[ "$(git -C "${FE_ROOT}" branch --show-current)" != "s2/r4-p0-local" ]]; then
  printf >&2 'Launch S2-R4 from s2/r4-p0-local.\n'
  exit 3
fi
...
if (( GPU_COUNT != 2 )); then
  printf >&2 'S2-R4 requires exactly two visible GPUs; found %d.\n' "${GPU_COUNT}"
  exit 3
fi
print_plan
fetch_candidate_branches
...
for index in "${!CANDIDATES[@]}"; do
  ...
  test -f "${path}/experiments/wam_flow/s2_r4/candidate.env"
  test -f "${path}/experiments/wam_flow/s2_r4/candidate_card.yaml"
  test -f "${path}/${CONFIG_REL}"
  WORKTREES+=("${path}")
done
(
  cd "${FE_ROOT}"
  uv run --frozen python scripts/validate_s2_r4_branch_pair.py \
    --p0-config "${WORKTREES[0]}/${CONFIG_REL}" \
    --p1-config "${WORKTREES[1]}/${CONFIG_REL}"
)

prompt_hf_token
python3 "${FE_ROOT}/scripts/s2_r4_runtime.py" init \
  --run-root "${RUN_ROOT}" \
  --run-id "${RUN_ID}" \
  --session "${SESSION}" \
  ...
  --base-repo "${FE_ROOT}" \
  --worktree "P0=${WORKTREES[0]}" \
  --worktree "P1=${WORKTREES[1]}"
...
prepare_command="$(shell_join \
  ...
  "S2_R4_READY_FILE=${READY_FILE}" \
  "S2_R4_FAILED_FILE=${FAILED_FILE}" \
  "S2_R4_HF_TOKEN_FIFO=${HF_TOKEN_FIFO}" \
  "S2_R4_P0_CONFIG=${WORKTREES[0]}/${CONFIG_REL}" \
  "S2_R4_FLOW_CHECKPOINT=${S2_R4_FLOW_CHECKPOINT:-}" \
  "S2_R4_R3_W1_CHECKPOINT=${S2_R4_R3_W1_CHECKPOINT:-}" \
  "UV_CACHE_DIR=${SHARED_UV_CACHE}" \
  "UV_PROJECT_ENVIRONMENT=${SHARED_UV_ENV}" \
  bash scripts/prepare_s2_r4_shared.sh
)"
create_persistent_window \
...
monitor_command="$(shell_join \
  python3 scripts/s2_r4_runtime.py monitor \
  --run-root "${RUN_ROOT}" \
  --interval 5
)"
MONITOR_WINDOW_ID="$(
...
printf '\nS2-R4 is running inside permanent tmux session %s.\n' "${SESSION}"
printf 'Monitor: tmux select-window -t %s:%s\n' "${SESSION}" "${MONITOR_WINDOW}"
printf 'One-shot: python3 scripts/s2_r4_runtime.py monitor --once --run-root %s\n' \
  "${RUN_ROOT}"
