#!/usr/bin/env python3
"""Evaluate an S2-R5 protected-team candidate on the fixed held-out split."""
...
from scripts.train_s2_r5_protected_team import (  # noqa: E402
    CHECKPOINT_FORMAT,
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (
        ...
        raise RuntimeError("S2-R5 evaluation requires exactly one visible GPU")
    saved = torch.load(checkpoint_path, map_location=device, weights_only=False)
    if saved.get("format_version") != CHECKPOINT_FORMAT:
        raise ValueError("checkpoint is not an S2-R5 protected-team predictor")
    ...
    if (
        ...
        or method.get("team_mixer") != team_mixer
        or method.get("protected_own") is not True
        or method.get("world_predictor_path") != "strictly_off_path"
    ):
        raise ValueError("S2-R5 checkpoint method differs from runtime config")
    ...
    if configured_model != model_config:
        raise ValueError("S2-R5 checkpoint/config model mismatch")
    team_model_config = ProtectedTeamFuturePredictorConfig.from_dict(
        dict(_mapping(saved, "team_model_config"))
    )
    configured_team_model = ProtectedTeamFuturePredictorConfig.from_dict(
        dict(_mapping(raw, "team_model"))
    )
    if configured_team_model != team_model_config:
        raise ValueError("S2-R5 team checkpoint/config model mismatch")
    model = ProtectedTeamFuturePredictor(
        model_config,
        team_model_config,
    ).to(device)
    protected = _mapping(saved, "protected_parent")
    p0_path = Path(str(protected["checkpoint"])).expanduser().resolve(strict=True)
    if file_sha256(p0_path) != protected.get("checkpoint_sha256"):
        raise ValueError("protected R4-P0 checkpoint hash changed")
    p0 = torch.load(p0_path, map_location=device, weights_only=False)
    if p0.get("format_version") != R4_CHECKPOINT_FORMAT:
        raise ValueError("protected parent is not an R4 checkpoint")
    model.load_protected_own(_mapping(p0, "model"))
    ...
    if artifact_sha256 != saved.get("future_artifacts_sha256"):
        raise ValueError("S2-R5 checkpoint PCA/statistics identity changed")
    ...
    with torch.inference_mode():
        for task_id, indices in selected.items():
            ...
            for batch_index, raw_batch in enumerate(loader, start=1):
                ...
                reference_own = model.protected_own(
                    current_state,
                    current_visual,
                    actions,
                    valid_agents,
                ...
                batch_own_exact = bool(
                    torch.equal(reference_own[0], normal_prediction.own_state)
                    and torch.equal(reference_own[1], normal_prediction.own_visual)
                )
                own_elementwise_exact = own_elementwise_exact and batch_own_exact
                own_max_abs_diff = max(
                    own_max_abs_diff,
                    float(
                        (reference_own[0] - normal_prediction.own_state)
                        .abs()
                        .max()
                    ),
                    float(
                        (reference_own[1] - normal_prediction.own_visual)
                        .abs()
                        .max()
                    ),
                )
                ...
                progress = {
                    ...
                    "program": "evaluate_s2_r5_protected_team.py",
                    ...
                    "protected_own_exact": batch_own_exact,
    ...
    comparison_contract = {
        ...
        "protected_p0_checkpoint_sha256": protected["checkpoint_sha256"],
        "protected_p0_model_sha256": protected["model_sha256"],
    ...
    payload = {
        ...
        "protected_own_evidence": {
            "elementwise_exact": own_elementwise_exact,
            "maximum_absolute_difference": own_max_abs_diff,
            "checkpoint_sha256": protected["checkpoint_sha256"],
            "model_sha256": protected["model_sha256"],
            "checkpoint_stable": (
                file_sha256(p0_path) == protected["checkpoint_sha256"]
            ),
            "optimizer_excluded": protected["optimizer_excluded"],
        },
...
def _action_equivalence_smoke(
    ...
    *,
    ...
    predictor_keys = set(_mapping(predictor_checkpoint, "team_model"))
    excluded = not any(
