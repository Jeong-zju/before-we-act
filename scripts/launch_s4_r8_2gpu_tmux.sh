...
WORKTREE_ROOT="${S4_R8_WORKTREE_ROOT:-${WORKSPACE_ROOT}/worktrees}"
RUN_ID="${S4_R8_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT=""
SESSION="ssh_tmux"
WINDOW_PREFIX=""
PREPARE_FROM_S0=0
...
CONFIG_RELS=(
  configs/wam_flow/s4_r8.yaml
  configs/wam_flow/s4_r8.yaml
)
PREPARE_REL=scripts/prepare_s4_r8_shared.sh
CANDIDATE_REL=scripts/run_s4_r8_candidate.sh
RUNTIME_REL=scripts/s4_r8_runtime.py
PAIR_VALIDATOR_REL=scripts/validate_s4_r8_branch_pair.py
...
fail() {
  printf >&2 'S4-R8 launcher error: %s\n' "$*"
  exit 3
}
...
RUN_ROOT="${S4_R8_RUN_ROOT:-${FE_ROOT}/outputs/s4_r8_runs/${RUN_ID}}"
WINDOW_PREFIX="${S4_R8_WINDOW_PREFIX:-${RUN_ID}}"
...
resolve_permanent_session() {
  ...
  if [[ -n "${TMUX:-}" ]]; then
    ...
    if [[ "${current}" != "${SESSION}" ]]; then
      fail "current tmux session is ${current}; S4-R8 must reuse ${SESSION}"
    fi
  fi
}
...
validate_existing_run() {
  ...
  if [[ "${manifest_round}" != s4-r8 || "${manifest_run}" != "${RUN_ID}" || \
        "${manifest_session}" != "${SESSION}" || "${manifest_prefix}" != "${WINDOW_PREFIX}" ]]; then
    fail "existing run manifest identity does not match requested S4-R8 run"
  fi
}
...
validate_launch_read_only() {
  ...
  if [[ "${gpu_count}" != 2 ]]; then
    fail "S4-R8 requires exactly two visible GPUs; found ${gpu_count:-0}"
  fi
  ...
  for script in "${PREPARE_REL}" "${CANDIDATE_REL}" "${RUNTIME_REL}" \
    "${PAIR_VALIDATOR_REL}" scripts/accept_s4_r8.py \
    scripts/evaluate_s4_r8_prefix_causality.py \
    scripts/prepare_shared_hdf5_receipt.py \
    scripts/prepare_s4_future_feature_cache.py; do
    if [[ ! -f "${FE_ROOT}/${script}" ]]; then
      fail "missing S4-R8 public-slice script: ${script}"
    fi
  done
  local branch
...
print_plan() {
  ...
  printf 'R8-axis=WUC fixed 0; prefix_mean vs rank32 causal_prefix_attention; no R7 candidate checkpoint dependency\n'
  printf 'R8-special=FP32 step0 exact + all source×horizon suffix exact + episode-bootstrap legal-prefix shuffle\n'
...
prepare_command="$(shell_join env -u HF_TOKEN \
  "S4_R8_RUN_ROOT=${RUN_ROOT}" "S4_R8_READY_FILE=${READY_FILE}" \
  "S4_R8_FAILED_FILE=${FAILED_FILE}" "S4_R8_HEARTBEAT_SECONDS=20" \
  "S4_R8_USE_S0_PREP=${PREPARE_FROM_S0}" \
  "S4_R8_HF_TOKEN_FIFO=${HF_TOKEN_FIFO}" \
...
for index in "${!CANDIDATES[@]}"; do
  candidate_command="$(shell_join env -u HF_TOKEN \
    "S4_R8_RUN_ROOT=${RUN_ROOT}" "S4_R8_HEARTBEAT_SECONDS=20" \
    "UV_CACHE_DIR=${UV_CACHE}" "UV_PROJECT_ENVIRONMENT=${UV_ENV}" \
    bash "${CANDIDATE_REL}" \
...
printf 'S4-R8 is running in permanent tmux %s.\n' "${SESSION}"
