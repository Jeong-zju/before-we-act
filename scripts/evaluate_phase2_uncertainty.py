"""Calibrate RWM-U uncertainty and emit an auditable Gate C report."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config", type=Path, default=ROOT / "configs/wam/phase2_rwm_u_v1.yaml"
    )
    parser.add_argument("--data-dir", type=Path)
    parser.add_argument("--checkpoint-dir", type=Path)
    parser.add_argument("--phase1-metrics", type=Path)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "outputs/wam_phase2_uncertainty_v1",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_paths["validation"] or not split_paths["test"]:
        raise RuntimeError("Gate C requires non-empty validation and test splits")
    ...
    with TrainingProgress(
        ...
        try:
            ...
            gate_c = _gate_c_report(
                evaluation,
                _read_json(settings["phase1_metrics"]),
                metadata,
                settings,
            ...
            test_phase.finish("Gate C " + ("passed" if gate_c["passed"] else "failed"))

            report_phase = progress.add_phase("write Gate C report", 2)
            report = {
                "format_version": "wam.phase2.uncertainty/1",
                "checkpoint": str(Path(settings["checkpoint_dir"]).resolve()),
                "phase1_metrics": str(Path(settings["phase1_metrics"]).resolve()),
                "calibration": calibration,
                "test": evaluation,
                "gate_c": gate_c,
            }
    ...
    print(json.dumps({"output": str(output_dir.resolve()), "gate_c": gate_c}, indent=2))
    return 0 if gate_c["passed"] else 2


def _gate_c_report(
    evaluation: Mapping[str, Any],
    phase1_metrics: Mapping[str, Any],
    ...
    *,
    ...
    horizon = str(settings["gate_c_horizon"])
    ...
    mlp_nrmse = float(
        phase1_metrics["test"]["models"]["phase0_mlp_recursive"][
            "exact_horizon"
        ][horizon]["continuous_state_nrmse"]
    )
    ...
    checks = {
        "h20_vs_phase0_mlp": {
            "passed": mlp_improvement >= settings["gate_c_mlp_improvement_min"],
            "ensemble_nrmse": ensemble_nrmse,
            "phase0_mlp_nrmse": mlp_nrmse,
            "relative_improvement": mlp_improvement,
            "minimum": settings["gate_c_mlp_improvement_min"],
        },
        "autoregressive_vs_teacher_forcing": {
            "passed": bool(
                ...
                and ar_improvement
                >= settings["gate_c_ar_over_teacher_forcing_min"]
            ),
            "member0_nrmse": member_nrmse,
            "teacher_forcing_nrmse": teacher_nrmse,
            "relative_improvement": ar_improvement,
            "minimum": settings["gate_c_ar_over_teacher_forcing_min"],
        },
        "uncertainty_error_correlation": {
            "passed": bool(
                exact["uncertainty_error_spearman"] is not None
                and exact["uncertainty_error_spearman"]
                >= settings["gate_c_uncertainty_error_spearman_min"]
            ),
            "spearman": exact["uncertainty_error_spearman"],
            "minimum": settings["gate_c_uncertainty_error_spearman_min"],
        },
        "ood_identification": {
            "passed": bool(
                ood["auroc"] is not None
                and ood["auroc"] >= settings["gate_c_ood_auroc_min"]
                and ood["epistemic_ratio"]
                >= settings["gate_c_ood_epistemic_ratio_min"]
            ),
            "auroc": ood["auroc"],
            "auroc_minimum": settings["gate_c_ood_auroc_min"],
            "epistemic_ratio": ood["epistemic_ratio"],
            "ratio_minimum": settings["gate_c_ood_epistemic_ratio_min"],
        },
        "event_aligned_no_average_braking": {
            "passed": bool(
                event["available"]
                and event["member_dominant_agent_accuracy"]
                >= settings["gate_c_event_dominant_agent_accuracy_min"]
                and event["member_ambiguous_braking_rate"]
                <= settings["gate_c_event_ambiguous_rate_max"]
            ),
            **event,
            "accuracy_minimum": settings["gate_c_event_dominant_agent_accuracy_min"],
            "ambiguous_rate_maximum": settings["gate_c_event_ambiguous_rate_max"],
        },
        "independent_members": {
...
def _markdown_report(report: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        (
            "# Phase 2 RWM-U uncertainty / Gate C report",
            "",
            f"结论：**Gate C {'通过' if gate['passed'] else '未通过'}**。",
            "",
            "| 判据 | 结论 | 关键数据 |",
            "|---|---|---|",
            *rows,
...
def _resolve_settings(
    ...
    return {
        ...
        "phase1_metrics": args.phase1_metrics
        or ROOT / config["phase1_reference"]["open_loop_metrics"],
        ...
        **{
            ...
            for name in (
                ...
                "ood_action_high",
                "gate_c_horizon",
                "gate_c_mlp_improvement_min",
                "gate_c_ar_over_teacher_forcing_min",
                "gate_c_uncertainty_error_spearman_min",
                "gate_c_ood_auroc_min",
                "gate_c_ood_epistemic_ratio_min",
                "gate_c_event_dominant_agent_accuracy_min",
                "gate_c_event_ambiguous_rate_max",
            )
        },
    }
