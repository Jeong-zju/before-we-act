#!/usr/bin/env python3
"""Audit the frozen MARS-Control RDT-1B policy/training contract."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        / "configs/rdt/mars_control_rdt1b_full_data_v1.json",
    )
    args = parser.parse_args()
    raw = args.contract.read_bytes()
    cfg = json.loads(raw)

    assert cfg["schema"] == "before-we-act.rdt.mars-control.full-data/1"
    ...
    assert trainer["max_train_steps"] == 300000
    ...
    assert data["total_episodes"] == 600
    assert data["local_arm_streams"] == 1650
    ...
    assert validation["total_episodes"] == 80
    assert validation["result"]["successes"] == 12
    assert validation["result"]["success_rate"] == 0.15
