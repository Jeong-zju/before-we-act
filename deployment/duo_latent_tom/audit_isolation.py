...
def main() -> None:
    ...
    qpos = torch.randn((2, 2, 8), device=args.device); task = torch.nn.functional.one_hot(torch.tensor([2, 2], device=args.device), num_classes=11).float(); arm_id = torch.tensor([[1.0, 0.0], [0.0, 1.0]], device=args.device)
    with torch.inference_mode():
        first = model._encode({"image": image, "qpos": qpos, "task": task, "arm_id": arm_id})
        second = model._encode({"image": image, "qpos": qpos, "task": task, "arm_id": arm_id})
    ...
    with torch.inference_mode():
        local_a = model._encode({"image": image, "qpos": qpos, "task": task, "arm_id": arm_id})[0]
        local_b = model._encode({"image": image_alt, "qpos": qpos_alt, "task": task, "arm_id": arm_id})[0]
    # Perturbing row 1 must not alter row 0; row 1 itself is intentionally
    # different because it received the perturbed local observation.
    cross_actor_diff = float((local_a[0] - local_b[0]).abs().max().cpu())
