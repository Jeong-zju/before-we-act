...
def main() -> None:
    ...
    obs = {"image": torch.zeros((1, 2, 3, 224, 448), dtype=torch.uint8, device=args.device), "qpos": torch.zeros((1, 2, 8), device=args.device), "task": torch.nn.functional.one_hot(torch.tensor([0], device=args.device), num_classes=11).float()}
    ...
    passed = output.shape == (1, 40, 8) and bool(torch.isfinite(output).all()) and not bool(model.scheduler.config.clip_sample)
    report = {"schema": "duobench.latent-tom.checkpoint-smoke.v1", "status": "complete" if passed else "failed", "checkpoint_step": int(saved["step"]), "shape": list(output.shape), "finite": bool(torch.isfinite(output).all()), "ddim_clip_sample": bool(model.scheduler.config.clip_sample), "policy_contract": POLICY_CONTRACT}
