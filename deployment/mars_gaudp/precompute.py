    ...
    for task in TASKS:
        ...
        if outpath.exists() and metap.exists() and not a.smoke:
            try:
                ...
                if old.get("status")=="complete": allmeta["tasks"][task]=old["rows"]; allmeta["indexed_local_timesteps"]+=sum(int(r["length"])*ARMS[task] for r in old["rows"]); continue
            except Exception: pass
        with h5py.File(outpath,"w") as out:
            for arm in range(ARMS[task]): out.create_dataset(f"gaussian_arm{arm}",shape=(0,13,*a.output_hw),maxshape=(None,13,*a.output_hw),dtype="float16",chunks=(1,13,*a.output_hw),compression="lzf")
            ...
            for shard in shards:
                    ...
                    for traj in sorted(source,key=lambda x:int(x.rsplit("_",1)[-1])):
                        ...
                        for arm in range(ARMS[task]):
                            ...
                            for begin in range(0,n,a.batch_size):
                                x=torch.from_numpy(images[begin:begin+a.batch_size]).permute(0,3,1,2).float(); x=F.interpolate(x,size=(256,256),mode="bilinear",align_corners=False).mul(2).sub(1).to(device)
                                with torch.inference_mode(), torch.autocast(device_type="cuda",dtype=torch.bfloat16): y=enc({"image":x[:,None]})[:,0].float()
                                y=F.interpolate(y,size=tuple(a.output_hw),mode="bilinear",align_corners=False).cpu().numpy().astype(np.float16); pieces.append(y)
                ...
                if a.smoke and len(rows)>=1: break
        atomic_json(metap,{"status":"complete","task":task,"rows":rows,"gaussian_hw":list(a.output_hw),"encoder_precision":"bfloat16_autocast","stored_dtype":"float16"}); allmeta["tasks"][task]=rows; allmeta["indexed_local_timesteps"]+=sum(int(r["length"])*ARMS[task] for r in rows)
