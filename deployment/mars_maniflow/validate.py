...
from .common import ACTION_HIGH, ACTION_LOW, TASKS, POLICY_CONTRACT, atomic_json
...
def main():
    ...
    for tid,spec in enumerate(TASKS):
        ...
        for ep in range(episodes):
            ...
            try:
                env=env_for(a.robofactory_root,spec); obs,_=env.reset(seed=seed); torch.manual_seed(seed); torch.cuda.manual_seed_all(seed); ih=[deque(maxlen=2) for _ in range(spec.arms)]; qh=[deque(maxlen=2) for _ in range(spec.arms)]; chunks=[None]*spec.arms; offsets=[a.replan_interval]*spec.arms
        ...
        report={'schema':'mars-control.maniflow.validation20.task.v2','status':'failed' if any('error' in x for x in rows) else 'complete','task':spec.name,'episodes':len(rows),'target_episodes':episodes,'successes':sum(int(x['success']) for x in rows),'success_rate':sum(int(x['success']) for x in rows)/len(rows),'max_steps':spec.max_steps,'seed_start':spec.seed_start,'policy_contract':POLICY_CONTRACT,'evaluator_revision':'maniflow-mars-temporal-actionclip-v2','preprocessing':{'rgb':'uint8_div_255_then_model_resize','qpos':'global_corpus_minmax_to_minus1_plus1','action':'env_clipped_before_training_minmax_inverse_once_then_env_clip'},'episodes_detail':rows}; atomic_json(out/f'{spec.name}.json',report); reports[spec.name]=report
    errors=[x for r in reports.values() for x in r['episodes_detail'] if 'error' in x]; summary={'schema':'mars-control.maniflow.validation20.v2','status':'failed' if errors else 'complete','baseline':'ManiFlow','benchmark':'MARS-Control','episodes_per_task':episodes,'total_episodes':episodes*len(TASKS),'successes':sum(r['successes'] for r in reports.values()),'macro_success_rate':float(np.mean([r['success_rate'] for r in reports.values()])),'tasks':reports,'policy_contract':POLICY_CONTRACT,'evaluator_revision':'maniflow-mars-temporal-actionclip-v2'}; atomic_json(out/'summary.json',summary)
