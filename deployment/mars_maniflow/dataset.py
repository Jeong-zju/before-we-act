...
def index_corpus(root, stats_path=None):
    ...
    action_low=np.asarray(ACTION_LOW,np.float64); action_high=np.asarray(ACTION_HIGH,np.float64)
            ...
            with h5py.File(path,'r') as f:
                for tr in sorted(f,key=lambda n:int(n.rsplit('_',1)[-1])):
                    ...
                    for arm in range(spec.arms):
                        q=np.asarray(g[f'obs/agent/panda-{arm}/qpos'][:n],np.float64); x=np.clip(np.asarray(g[f'actions/panda-{arm}'][:n],np.float64),action_low,action_high); im=g[f'obs/sensor_data/head_camera_agent{arm}/rgb']
...
class MarsManiFlowDataset(Dataset):
    ...
    def __getitem__(self,i):
        tid,path,tr,arm,n,t=self.entries[i]; g=self._handle(path)[tr]
        # Match ManiFlow's upstream pad_before=n_obs_steps-1 contract:
        # [obs(t-1), obs(t)] supervises [action(t-1), action(t), ...].
        start=t-(self.obs_steps-1); end=start+self.horizon
        obs_lo,obs_hi=max(0,start),min(n,t+1); act_lo,act_hi=max(0,start),min(n,end)
        ims=np.asarray(g[f'obs/sensor_data/head_camera_agent{arm}/rgb'][obs_lo:obs_hi],np.uint8); q=np.asarray(g[f'obs/agent/panda-{arm}/qpos'][obs_lo:obs_hi],np.float32); x=np.clip(np.asarray(g[f'actions/panda-{arm}'][act_lo:act_hi],np.float32),ACTION_LOW,ACTION_HIGH)
        if len(ims)<self.obs_steps: ims=np.concatenate([np.repeat(ims[:1],obs_lo-start,0),ims],0)
        if len(q)<self.obs_steps: q=np.concatenate([np.repeat(q[:1],obs_lo-start,0),q],0)
        if len(x)<self.horizon: x=np.concatenate([np.repeat(x[:1],act_lo-start,0),x,np.repeat(x[-1:],end-act_hi,0)],0)
