...
ROOT=Path(os.environ.get("MARS_DP_REPO","/workspace/repos/before-we-act")); RF=Path(os.environ.get("MARS_DP_ROBOFACTORY","/workspace/repos/RoboFactory")); DATA=Path(os.environ.get("MARS_DP_DATA_ROOT","/workspace/datasets/mars_control")); RUN=Path(os.environ.get("MARS_DP_RUN_ROOT","/workspace/runs/mars_dp")); PY=os.environ.get("MARS_DP_PYTHON","/venv/main/bin/python"); active=None; stopping=False
def now(): return datetime.now(timezone.utc).isoformat()
def state(stage,status="running",**kw): atomic_json(RUN/"state.json",{"schema":"mars-control.dp.supervisor.v1","stage":stage,"status":status,"updated_at":now(),**kw})
def finished(name):
