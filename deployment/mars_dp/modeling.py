...
def build_policy(stats, device="cpu"):
    shape = {"obs": {"head_cam": {"shape": [3, 240, 320], "type": "rgb"}, "agent_pos": {"shape": [8], "type": "low_dim"}}, "action": {"shape": [8]}}
...
def load_policy(checkpoint, device="cuda:0", inference_steps=20, weights="ema"):
    ...
    if payload.get("contract") != "shared_weights_strict_local_rgb_own_command_state_to_absolute_action8": raise RuntimeError("checkpoint contract mismatch")
    key = {"ema": "ema_model", "online": "model"}.get(weights)
    if key is None: raise ValueError(f"unknown weight set: {weights}")
    policy = build_policy(payload["stats"], device); policy.load_state_dict(payload[key], strict=True); policy.normalizer = policy.normalizer.to(device); policy.num_inference_steps = int(inference_steps); policy.eval()
