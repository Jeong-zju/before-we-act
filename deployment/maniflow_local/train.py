...
from local_dataset import LocalManiFlowDataset
...
def checkpoint_payload(model, ema, optimizer, lr_scheduler, step, args, dataset_size):
    return {
        ...
        "dataset_size": dataset_size, "all_episodes": True, "seed": args.seed,
...
def main():
    ...
    p.add_argument("--dataset-root", default=os.getenv("BWA_DATASET_ROOT", "/workspace/datasets/robofactory_multitask"))
    p.add_argument("--output", default=os.getenv("BWA_MANIFLOW_OUTPUT", "/workspace/bwa_maniflow_runs/formal"))
    p.add_argument("--steps", type=int, default=int(os.getenv("MANIFLOW_STEPS", "100200")))
    p.add_argument("--batch-size", type=int, default=int(os.getenv("MANIFLOW_BATCH", "16")))
    ...
    dataset = LocalManiFlowDataset(args.dataset_root)
    sampler = LocalityBatchSampler(len(dataset), args.batch_size, args.seed, args.locality)
    ...
    for step in range(start + 1, args.steps + 1):
        ...
        if step % args.save_every == 0 or step == args.steps:
            atomic_torch(latest, checkpoint_payload(model, ema, optimizer, lr, step, args, len(dataset)))
    log.close()
    status = {"schema": "bwa.maniflow.training_status.v1", "status": "complete", "step": args.steps,
              "all_episodes": True, "episodes": 900, "dataset_size": len(dataset),
