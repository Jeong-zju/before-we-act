...
class FrameIndexedPAIRWristDataset(Dataset):
    """All-3-style weighted items without decoding entire episodes into RAM."""

    def __init__(self, trajectories, horizon, stats, train):
        self.horizon, self.stats = horizon, stats
        ...
        for path, key, length, present, task in kept:
            ...
            for arm in present:
                ...
                for time_index in range(length):
                    ...
                    self.items.append((path, key, time_index, arm, task))
                streams.append(tuple(indices))
            self.episodes.append((task, tuple(streams), length))
    ...
    def __getitem__(self, request):
        if isinstance(request, tuple):
            ...
            index, group = request, -1
        path, key, time_index, arm, _task = self.items[index]
        with h5py.File(path, "r") as handle:
            trajectory = handle[key]
            sensor = trajectory["obs"]["sensor_data"][f"head_camera_agent{arm}"]
            rgb = np.asarray(sensor["rgb"][time_index])
            depth = np.asarray(sensor["depth"][time_index])
            qpos = np.asarray(trajectory["obs"]["agent"][f"panda-{arm}"]["qpos"][time_index],
                              dtype=np.float32)
            action_dataset = trajectory["actions"][f"panda-{arm}"]
            end = min(time_index + self.horizon, len(action_dataset))
            future = np.asarray(action_dataset[time_index:end], dtype=np.float32)
        if tuple(rgb.shape) != (480, 640, 3) or tuple(depth.shape) != (480, 640, 1):
            raise ValueError(f"strict 640x480 RGB-D required for {path}:{key}:panda-{arm}")
        valid = len(future)
        padded = np.empty((self.horizon, future.shape[1]), np.float32)
        ...
        return (torch.from_numpy(rgb).permute(2, 0, 1).contiguous(),
                torch.from_numpy(depth).permute(2, 0, 1).contiguous(),
...
class MixedTaskTeamBatchSampler(Sampler):
    """Five tasks in every batch while preserving synchronized team groups.

    Each task contributes exactly eight local samples. For 2/4-agent tasks all
    eight samples belong to complete teams. For each 3-agent task, six samples
    form two synchronized teams and two additional local samples are marked -1
    and excluded only from the relation loss. Thus 36/40 samples retain valid
    synchronized relation supervision without any task-level update block.
    """

    def __init__(self, dataset, updates, seed):
        self.dataset, self.updates, self.seed, self.epoch = dataset, updates, seed, 0
        self.by_task = {}
        for episode in dataset.episodes:
            self.by_task.setdefault(episode[0], []).append(episode)
        self.tasks = sorted(self.by_task)
    ...
    def __iter__(self):
        rng = random.Random(self.seed + self.epoch); self.epoch += 1
        for _update in range(self.updates):
            batch, group = [], 0
            for task in self.tasks:
                episode = self.by_task[task][rng.randrange(len(self.by_task[task]))]
                _task, streams, length = episode
                complete, remainder = divmod(8, len(streams))
                for _ in range(complete):
                    time_index = rng.randrange(length)
                    batch.extend((stream[time_index], group) for stream in streams)
                    group += 1
                for _ in range(remainder):
                    extra_episode = self.by_task[task][rng.randrange(len(self.by_task[task]))]
                    _extra_task, extra_streams, extra_length = extra_episode
                    stream = extra_streams[rng.randrange(len(extra_streams))]
                    batch.append((stream[rng.randrange(extra_length)], -1))
            if len(batch) != 40:
                raise RuntimeError(f"mixed-team batch invariant failed: {len(batch)}")
            rng.shuffle(batch)
            yield batch
...
def main():
    ...
    p.add_argument("--sampler-mode", choices=("team_block", "weighted_items", "mixed_team_items"), default="team_block")
    ...
    if args.sampler_mode == "weighted_items":
        if args.relation_weight > 0:
            raise ValueError("weighted item sampling has no synchronized groups; relation_weight must be zero")
        dataset = FrameIndexedPAIRWristDataset(trajectories, 100, stats, True)
        generator = torch.Generator().manual_seed(args.seed)
        sampler = WeightedRandomSampler(torch.as_tensor(dataset.item_weights, dtype=torch.double),
    ...
    elif args.sampler_mode == "mixed_team_items":
        dataset = FrameIndexedPAIRWristDataset(trajectories, 100, stats, True)
        sampler = MixedTaskTeamBatchSampler(dataset, args.updates, args.seed)
    ...
    sample = dataset[0] if args.sampler_mode != "team_block" else dataset[(0, 0, 0)]
    state_dim, action_dim = len(sample[2]), len(sample[3][0])
    ...
    config = vars(args) | {
        ...
        "parallelism": f"single RTX 5090 {args.gpu_label}", "global_batch": args.batch_size,
        "sample_budget": args.batch_size * args.updates,
        "optimizer_updates": args.updates, "horizon": 100, "enc_layers": 4, "dec_layers": 7,
        ...
        "three_agent_batch_contract": (
            "39 samples in 13 complete teams + 1 local-action-only sample excluded from relation loss"
            if args.sampler_mode == "team_block" else
            "not applicable: relation supervision disabled for weighted local items"
            if args.sampler_mode == "weighted_items" else
            "per 3-agent task: 6 synchronized samples in 2 teams + 2 local-only; 36/40 total batch samples synchronized"),
        "task_sampling": (
            "equal 64-update task blocks over all five tasks"
            if args.sampler_mode == "team_block" else
            "All-3-style hierarchical task-balanced WeightedRandomSampler; tasks interleaved within every batch"
            if args.sampler_mode == "weighted_items" else
            "team-aware hierarchical task-balanced sampling; exactly 8 samples per task in every update; no task block"),
    }
