"""Frozen components for Step 3-N1-R1 action-grounded belief experiments.
...
from before_we_act.b3_n1_model import MatchedActionProbe, N1RawSignalModel
...
R1_SEEDS = (20260815, 20260816, 20260817)
R1_DATA_SEED = 20260815
R1_CAPACITY = 16
R1_MAX_UPDATES = 80_000
R1_MIN_UPDATES = 25_000
R1_EARLIEST_PLATFORM = 35_000
R1_EVAL_EVERY = 5_000
R1_LR_DROP = 20_000
R1_CONDITIONS = (
    "h",
...
class R1BalancedBatchSampler(Sampler[list[N1Request]]):
    ...
    def __iter__(self) -> Iterable[list[N1Request]]:
...
class R1OracleDataset(N1RawSignalDataset):
...
class FrozenR1Features:
    h: torch.Tensor
    belief: torch.Tensor
    history: torch.Tensor | None = None


class FrozenR1Backbones(nn.Module):
    """Read-only B0-H history hidden and full-token N1 belief extractor."""

    def __init__(
        self,
        *,
        b0h_checkpoint: str | Path,
        n1_checkpoint: str | Path,
        visual_mean: torch.Tensor,
        visual_std: torch.Tensor,
    ) -> None:
        super().__init__()
        b0h_payload = torch.load(b0h_checkpoint, map_location="cpu", weights_only=False)
        self.b0h = B0HPolicy(**_constructor_config(b0h_payload))
        self.b0h.load_state_dict(b0h_payload["model"], strict=True)
        n1_payload = torch.load(n1_checkpoint, map_location="cpu", weights_only=False)
        model_config = dict(n1_payload["model_config"])
        model_config.pop("capacities", None)
        self.n1 = N1RawSignalModel(**model_config)
        self.n1.load_state_dict(n1_payload["real_model"], strict=True)
    ...
    @torch.no_grad()
    def forward(self, batch: Mapping[str, torch.Tensor]) -> FrozenR1Features:
        self.eval()
        n1_inputs = {key: batch[key] for key in N1RawSignalDataset.RUNTIME_FIELDS}
        n1_output = self.n1(**n1_inputs)
        raw_visual = self._raw_visual(batch)
        task_index = batch["task_index"].long()
        history, h, _ = self.b0h._encode_history(
            raw_visual,
            raw_visual[:, -1],
            batch["history_qpos"],
        ...
        return FrozenR1Features(
            h=h,
            belief=n1_output.capacities[R1_CAPACITY].tokens,
            history=history,
        )


class CrossAttentionActionHead(nn.Module):
    def __init__(self, d_model: int = 384, tokens: int = R1_CAPACITY) -> None:
        super().__init__()
...
class R1FairProbeSet(nn.Module):
...
class R1OracleProbeSet(nn.Module):
    def __init__(self, d_model: int = 384) -> None:
        super().__init__()
        self.h = MatchedActionProbe(d_model)
    ...
    def parameter_counts(self) -> dict[str, int]:
        return {
            name: sum(parameter.numel() for parameter in getattr(self, name).parameters())
            for name in R1_ORACLE_CONDITIONS
        }


def oracle_mapping(batch: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    return {key: batch[key] for key in R1OracleDataset.ORACLE_ONLY_FIELDS}
...
def oracle_predictions(
    probes: R1OracleProbeSet,
    frozen: FrozenR1Features,
...
def all_oracle_conditions_platform(metrics: Sequence[Mapping]) -> bool:
    if len(metrics) < 4 or int(metrics[-1]["update"]) < R1_EARLIEST_PLATFORM:
        return False
...
def all_conditions_platform(metrics: Sequence[Mapping]) -> bool:
    return all(condition_platform(metrics, condition) for condition in R1_CONDITIONS)
