"""Closed-loop Validation5 evaluator for N2 and its frozen B0-H control."""
...
from before_we_act.evaluate_step2_b0h import (
    EpisodeHistory,
    TemporalChunkEnsembler,
    load_model as load_b0h,
    prepare_current,
    reset_reproducibly,
)
from before_we_act.team_belief.n2_core import B3N2Config


def load_n2(checkpoint: str, device: torch.device):
    ...
    config = saved["config"]
    n2_values = dict(config["n2_config"])
    for key in ("future_offsets_steps", "future_offsets_seconds"):
        if key in n2_values:
            n2_values[key] = tuple(n2_values[key])
    n2_config = B3N2Config(**n2_values)
    model = B3N2Policy(
        n2_config,
...
@torch.no_grad()
def predict_n2(model, stats, observation, arms, history, task, device):
...
@torch.no_grad()
def predict_b0h(model, stats, observation, arms, history, task, device):
    global_rgb, local_rgb, qpos = prepare_current(observation, arms, stats, device)
    temporal = history.batch(qpos, task, device)
    with torch.autocast("cuda", dtype=torch.bfloat16):
...
def evaluate(checkpoint, task_name, seeds, device_name, max_steps, mode):
    ...
    if mode == "n2":
        model, stats, config = load_n2(checkpoint, device)
        predictor = predict_n2
    else:
        model, stats, config = load_b0h(checkpoint, device)
        predictor = predict_b0h
