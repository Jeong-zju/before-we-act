"""Full 22,475-timestep held-out evaluation for an R12-R4 candidate."""
...
def main() -> None:
    ...
    if args.batch_size < 1 or args.workers < 0:
        raise ValueError("R12-R4 offline batch/workers differ")
    ...
    if (
        ...
        raise ValueError("R12-R4 offline checkpoint identity differs")
    ...
    if sha256(belief_path) != str(config.raw["belief_checkpoint_sha256"]):
        raise ValueError("R12-R4 offline W11 identity differs")
    ...
    if (
        ...
        raise ValueError("R12-R4 full validation index differs")
    stats = {
    ...
    if len(dataset) != sum(EXPECTED_VALIDATION.values()):
        raise ValueError("R12-R4 validation is not full-timestep")
    sampler = SequentialFullEpisodeSampler(dataset, args.batch_size)
    loader = DataLoader(
        dataset,
    ...
    for batch_index, cpu_batch in enumerate(loader):
        ...
        steps = batch["action_step_mask"][:, :, None, None]
        valid = active & steps
        ...
        for task_index, task in enumerate(TASKS):
            ...
            row["full_squared_error"] += float(selected_error[full_mask].sum().cpu())
        ...
        if heartbeat and (batch_index == 0 or batch_index % 50 == 0):
            ...
            atomic_json(
                heartbeat,
                {
                    "producer": "evaluate_action_generator_r4_offline",
    ...
    result = {
        ...
        "per_task": per_task,
        "observation": "native 480x640 RGB -> frozen DINO 30x40 -> post-encoder 6x8 cache + W11",
        "elapsed_seconds": time.monotonic() - started,
        "created_at": now(),
    }
    if not finite or observed != sum(EXPECTED_VALIDATION.values()):
        raise ValueError("R12-R4 full validation failed completeness/finite checks")
