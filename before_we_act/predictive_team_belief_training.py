"""Train-time wrappers and the pre-registered reactive control."""
...
@dataclass
class TeamBeliefExperimentOutput:
    candidate: TeamBeliefPolicyOutput
    direct_prediction: torch.Tensor
    direct_residual: torch.Tensor
    direct_gate: torch.Tensor


class TeamBeliefExperiment(nn.Module):
    """The complete team-belief candidate and its matched direct control."""
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> TeamBeliefExperimentOutput:
        ...
        return TeamBeliefExperimentOutput(
            candidate=candidate,
...
def paired_permutation(pair_id: torch.Tensor) -> torch.Tensor:
    ...
    if any(len(indices) != 2 for indices in rows.values()):
        raise ValueError("every paired-situation id must occur exactly twice")
