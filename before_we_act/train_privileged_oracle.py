"""Train one seed of the conditional privileged teammate oracle."""
...
def evaluate_oracle(backbones, probes, loader, device, *, include_samples=False):
    ...
    scores = {condition: [] for condition in PRIVILEGED_ORACLE_CONDITIONS}
    tasks = {condition: {task: [] for task in range(6)} for condition in PRIVILEGED_ORACLE_CONDITIONS}
    samples = []
    for raw in loader:
...
def main() -> None:
    ...
    if oracle_contract.get("status") != "FROZEN_BEFORE_F0_F1" or args.seed not in BELIEF_SEEDS:
    ...
    probes = PrivilegedOracleProbeSet().to(device)
    optimizer = torch.optim.AdamW(probes.parameters(), lr=args.lr, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda step: 1.0 if step < BELIEF_LR_DROP else 0.1)
    ...
    sampler = ActionGroundedBatchSampler(dataset.episodes, split, updates=BELIEF_MAX_UPDATES, data_seed=BELIEF_DATA_SEED, start_update=start)
