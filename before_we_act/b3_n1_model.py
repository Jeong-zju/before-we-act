"""Compact raw-signal representation and matched action probes for Step 3-N1."""
...
@dataclass
class N1RepresentationOutput:
    history: torch.Tensor
    history_summary: torch.Tensor
    task_embedding: torch.Tensor
    capacities: Mapping[int, N1CapacityOutput]


class N1RawSignalModel(nn.Module):
    ...
    def forward(
        ...
        valid = history_mask | action_history_mask
        outputs: dict[int, N1CapacityOutput] = {}
        for capacity in self.capacities_n:
            key = str(capacity)
            ...
            outputs[capacity] = N1CapacityOutput(
                tokens=tokens,
        ...
        return N1RepresentationOutput(history, history_summary, task, outputs)
...
class N1ActionProbeSet(nn.Module):
