"""Train one omniscient belief-teacher seed and its zero-privilege control."""
...
from before_we_act.raw_team_signal_model import MatchedActionProbe, masked_mse
...
def platform(metrics):
    if len(metrics)<4 or metrics[-1]["update"]<BELIEF_EARLIEST_PLATFORM:return False
...
def main():
    ...
    if contract.get("status")!="FROZEN_BEFORE_F0_F1" or args.seed not in BELIEF_SEEDS:raise RuntimeError("invalid teacher contract")
    ...
    dataset=ActionGroundedDataset(args.cache);signal_checkpoint=parent["old_n1_read_only"]["representation_checkpoints"][str(args.seed)]
    backbones=FrozenBeliefBackbones(temporal_checkpoint=Path(parent["b0h"]["checkpoint"]),signal_checkpoint=Path(signal_checkpoint["path"]),visual_mean=dataset.visual_mean,visual_std=dataset.visual_std).to(device)
    ...
    parameters=list(teacher.parameters())+list(matched.parameters());optimizer=torch.optim.AdamW(parameters,lr=args.lr,weight_decay=1e-4);scheduler=torch.optim.lr_scheduler.LambdaLR(optimizer,lambda step:1.0 if step<BELIEF_LR_DROP else .1)
    ...
    sampler=ActionGroundedBatchSampler(dataset.episodes,split,updates=BELIEF_MAX_UPDATES,data_seed=BELIEF_DATA_SEED,start_update=start);loader=DataLoader(dataset,batch_sampler=sampler,num_workers=args.workers,pin_memory=True,persistent_workers=args.workers>0,prefetch_factor=2 if args.workers>0 else None);validation=fixed_loader(dataset,split,"validation")
    ...
    for update,raw in enumerate(loader,start=start+1):
        ...
        if update%BELIEF_EVAL_EVERY:continue
        ...
        if update>=BELIEF_MAX_UPDATES:completion="INCONCLUSIVE_TRAINING_NOT_CONVERGED";break
