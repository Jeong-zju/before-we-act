"""Train matched action probes on a frozen raw team-signal representation."""
...
from before_we_act.raw_team_signal_model import TeamActionProbeSet, RawTeamSignalEncoder, masked_mse
from before_we_act.train_raw_team_signal import (
    EARLIEST_PLATFORM,
    EVAL_EVERY,
    LR_DROP,
    MAX_UPDATES,
...
def evaluate(representation, probes, loader, device: torch.device) -> dict:
    ...
    conditions = (*TeamActionProbeSet.CONDITIONS, "phase_shuffle")
...
def platform(metrics: list[dict]) -> bool:
    if len(metrics) < 4 or metrics[-1]["update"] < EARLIEST_PLATFORM:
        return False
    recent = metrics[-4:]
    for condition in TeamActionProbeSet.CONDITIONS:
        for capacity in CAPACITY_CANDIDATES:
...
def main() -> None:
    ...
    representation = RawTeamSignalEncoder(**{key: value for key, value in representation_payload["model_config"].items() if key != "capacities"}).to(device)
    ...
    probes = TeamActionProbeSet().to(device)
