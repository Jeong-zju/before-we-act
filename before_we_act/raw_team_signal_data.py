"""Frozen data projection for raw team-signal experiments.
...
class RawTeamSignalDataset(Dataset):
    ...
    def __init__(self, cache_root: str | Path) -> None:
        ...
        self.metadata, self.episodes = load_team_signal_metadata(self.root)
...
class BalancedTeamBatchSampler(Sampler[list[TeamSampleRequest]]):
    ...
    def requests_for_update(self, update: int) -> list[TeamSampleRequest]:
        ...
        for task in SIX_TASKS:
            for _ in range(SAMPLES_PER_TASK):
                ...
                requests.append(TeamSampleRequest(episode_index, arm, time_index, sample_key, task))
        rng.shuffle(requests)
        if len(requests) != EFFECTIVE_BATCH:
            raise AssertionError("3-N1 effective batch drift")
        return requests

    def __iter__(self) -> Iterator[list[TeamSampleRequest]]:
...
def validation_requests(cache_root: str | Path) -> list[TeamSampleRequest]:
    root = Path(cache_root)
    metadata, _ = load_team_signal_metadata(root)
    return [TeamSampleRequest(**row) for row in metadata["validation_requests"]]
