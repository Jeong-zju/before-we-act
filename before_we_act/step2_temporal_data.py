"""Frozen Step-2 temporal sample contract over the original 720 demonstrations.
...
def episode_receipt(episodes: Sequence[Step2Episode]) -> dict:
...
class _VisualCache:
    ...
    def path_for(self, episode: Step2Episode) -> Path:
        return self.root / episode.task / f"{episode.hdf5_sha256}.npz"

    def load(self, episode: Step2Episode) -> dict[str, np.ndarray]:
...
class TeamTemporalDataset(Dataset):
    ...
    def __init__(
        self,
        episodes: Sequence[Step2Episode],
