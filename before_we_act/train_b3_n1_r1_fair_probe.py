"""Train one seed of the frozen R1-1 fair H+B versus true B0-H probe."""
...
def evaluate_fair(
    ...
    *,
    ...
    backbones.eval()
    probes.eval()
    scores = {condition: [] for condition in R1_CONDITIONS}
    tasks = {
        condition: {task: [] for task in range(6)} for condition in R1_CONDITIONS
    }
    groups = {condition: {} for condition in R1_CONDITIONS}
...
def main() -> None:
    ...
    n1_checkpoint = Path(paths["path"])
    b0h_checkpoint = Path(contract["b0h"]["checkpoint"])
    if sha256_file(n1_checkpoint) != paths["sha256"]:
        raise RuntimeError("frozen N1 checkpoint hash differs")
    if sha256_file(b0h_checkpoint) != contract["b0h"]["checkpoint_sha256"]:
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lambda step: 1.0 if step < R1_LR_DROP else 0.1
    )
