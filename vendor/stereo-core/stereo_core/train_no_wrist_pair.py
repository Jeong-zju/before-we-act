"""Train no-wrist global+agent RGB Stereo-CoRE on WAM trajectory HDF5."""
...
def load_episodes(manifest_paths: list[Path], split: str = "train") -> list[dict]:
    ...
    if seen_tasks != set(CANONICAL_TASKS):
        raise ValueError(f"expected all five tasks, got {sorted(seen_tasks)}")
...
class NoWristFrameDataset(Dataset):
    ...
    def __getitem__(self, request):
        ...
        with h5py.File(episode["path"], "r") as handle:
            ...
            local_rgb = np.asarray(images[f"agent_{arm}"][time_index])
            qpos = np.asarray(
...
class ExactFiveTaskBatchSampler(Sampler):
    ...
    def __init__(self, episodes: list[dict], updates: int, seed: int, start_update: int = 0):
        ...
        if set(self.by_task) != set(CANONICAL_TASKS):
            raise ValueError(f"expected five task buckets, got {sorted(self.by_task)}")
...
def main() -> None:
    ...
    if args.batch_size != 40:
        raise ValueError("locked protocol requires exact batch size 40")
    ...
    sampler = ExactFiveTaskBatchSampler(episodes, args.updates, args.seed, start_update)
    loader = DataLoader(
        dataset,
    ...
    config = vars(args) | {
        ...
        "task_sampling": "exactly 8 hierarchical episode/arm/time samples per task per batch",
    ...
    print(json.dumps({"complete": True, "update": args.updates}), flush=True)
