...
RUNNER = ROOT / "scripts/run_s4_r8_candidate.sh"


def _embedded_python() -> list[str]:
    source = RUNNER.read_text(encoding="utf-8")
    return [
        remainder.partition("\nPY\n")[0] for remainder in source.split("<<'PY'\n")[1:]
    ]
...
def test_candidate_runner_embedded_python_is_syntax_valid() -> None:
    ...
    for index, (block, remainder) in enumerate(zip(blocks, remainders, strict=True), 1):
        ...
        compile(block, f"run_s4_r8_candidate-heredoc-{index}", "exec")
...
def test_minimal_oom_preflight_is_terminal_but_not_a_pass(tmp_path: Path) -> None:
    verifier = next(
        ...
        if "preflight report must be a JSON object" in block
    )
    report = tmp_path / "preflight.json"
    report.write_text(
        json.dumps(
            {
                "format_version": "wam.robofactory.s4_r8.preflight/1",
                "identity": {
                    "round_id": "s4-r8",
                    "candidate_id": "P0",
                    "model_kind": "s4_r8_horizon_prefix_mean",
    ...
    common = [
        ...
        str(report),
        "P0",
        "s4_r8_horizon_prefix_mean",
        "a" * 64,
        "4",
        "3",
    ]
    terminal = subprocess.run(
        [*common, "terminal", "prefix_mean"],
        input=verifier,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
...
def test_checkpoint_verifier_requires_canonical_hash_and_resume_format(
    ...
    torch.save(
        {
            "format_version": "wam.robofactory.s4_r8.horizon_causal.checkpoint/1",
            "update": 30000,
            "method": method,
    ...
    torch.save(
        {
            "format_version": "wam.robofactory.s4_r8.horizon_causal.resume/1",
            "update": 1000,
            "identity": {
                **method,
...
def test_candidate_runner_orders_pair_exact_before_formal_training() -> None:
    ...
    formal_training = source.index("run_stage training train_s4_r8_horizon_causal.py")
    evaluation = source.index("run_stage validating evaluate_s4_r8_causal.py")
...
def test_candidate_runner_publishes_exact_program_pid_and_wait_heartbeats() -> None:
    ...
    assert (
        '--detail "${CURRENT_DETAIL}" --pid "$$" --child-pid "${child_value}"' in source
    )
...
def test_candidate_runner_exports_isolated_rollout_runtime() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    assert 'S4_R8_ROBOFACTORY_ROOT="${ROBOFACTORY_ROOT}"' in source
    assert 'S4_R8_RF_PYTHON="${RF_PYTHON}"' in source
