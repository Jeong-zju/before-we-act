...
from scripts.train_world_model_ensemble import _training_source_hashes
...
def test_ensemble_config_matches_rwm_u_contract():
    payload = yaml.safe_load(
        (ROOT / "configs/wam/world_model_ensemble.yaml").read_text(encoding="utf-8")
    )
    assert payload["pipeline"] == "world_model_ensemble"
    ...
    assert payload["evaluation"]["acceptance_horizon"] == 20
    assert payload["evaluation"]["baseline_mlp_h20_nrmse"] > 0.0
    assert payload["initialization"]["checkpoint"] == "checkpoints/world_model"


def test_ensemble_training_provenance_fingerprints_source_files():
    hashes = _training_source_hashes(ROOT / "configs/wam/world_model_ensemble.yaml")

    assert "configs/wam/world_model_ensemble.yaml" in hashes
    assert "scripts/train_world_model_ensemble.py" in hashes
...
def test_rwm_u_checkpoint_roundtrip_is_strict_and_loads_ablation(tmp_path: Path):
    ...
    save_rwm_u_checkpoint(
        ...
        experiment_config={"pipeline": "world_model_ensemble"},
