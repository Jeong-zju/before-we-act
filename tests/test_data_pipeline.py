...
from scripts.audit_dataset import main as audit_main
...
def test_proprio_collection_exports_aligned_actions_and_metadata_without_images(
    ...
    with h5py.File(path, "r") as file:
        ...
        assert file.attrs["behavior_id"] in dict(BEHAVIOR_WEIGHTS)
    ...
    assert manifest["behavior_profile"] == "mixed_proprio"


def test_baseline_mixture_schedule_matches_section_11_2_exactly():
    policy = CooperativeStopCollectionPolicy(
        object(), profile="mixed_proprio", mixture_seed=17
    )
    behaviors = [
    ...
    assert counts == Counter(dict(BEHAVIOR_WEIGHTS))
    for behavior in behaviors:
        json.dumps(behavior.perturbation_config)


def test_dataset_audit_accepts_complete_mixed_proprio_contract(tmp_path):
...
def test_baseline_models_and_training_entrypoint_smoke(tmp_path):
