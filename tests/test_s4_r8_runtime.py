...
import scripts.s4_r8_runtime as s4_r8_runtime
from scripts.s4_r8_runtime import (
    FORMAT_VERSION,
    initialize_run,
    render_monitor,
...
def _initialize(tmp_path: Path) -> Path:
    ...
    initialize_run(
        ...
        run_id="s4-r8-test",
        session="ssh_tmux",
        window_prefix="s4-r8-test",
        monitor_window="s4-r8-test-monitor",
        base_repo=tmp_path,
        parent_commit="a" * 40,
...
def test_r7_monitor_reports_runtime_heartbeat_progress_and_derived_gates(
    ...
    update_shared_status(
        root,
        phase="complete",
        program="prepare_s4_r8_shared.sh",
    ...
    for candidate, gpu in (("P0", 0), ("P1", 1)):
        update_status(
            ...
            candidate=candidate,
            phase="validating",
            program="evaluate_s4_r8_causal.py",
        ...
        _write_json(
            candidate_root / "validation/prefix_suffix_exact.json",
            {
                "groups": {
                    f"{source}@{horizon}": {
                        "suffix_elementwise_exact": True,
                        "legal_prefix_changes_output": True,
                    }
    ...
    assert "WAM S4-R8 monitor" in rendered
    ...
    assert (
        "condition=world_evidence_gate=0 task=take_photo episode=7/20 step=125/1500"
        in rendered
    )
    ...
    assert "P1 R8 prefix causality | suffix+prefix-sensitive=12/12" in rendered
    assert "bootstrap-lower>0=12/12 min-lower=0.01" in rendered
...
def test_r7_monitor_exposes_complete_normal_results_before_final_report(
    ...
    for task in tasks:
        summary[task] = {
            "episodes": [
                {"seed": 900 + index, "success": index < 9} for index in range(20)
            ]
        }
    _write_json(
        root / "candidates/p0/validation/gate20/normal/gate_summary.json",
        summary,
    )
...
def test_r7_monitor_replaces_zero_gpu_pid_sentinel_with_live_process(
    ...
    monkeypatch.setattr(s4_r8_runtime, "_gpu_processes_by_index", lambda: {0: [456]})
...
def test_r7_monitor_marks_nonterminal_heartbeat_stale_after_75_seconds(
    ...
    assert "STALE | last_program=train_s4_r8_horizon_causal.py" in rendered
...
def test_r7_shell_contracts_are_permanent_tmux_scoped_and_cli_driven() -> None:
    ...
    assert 'grep -Fzqx "S4_R8_RUN_ROOT=${RUN_ROOT}"' in stopper
...
def test_launcher_dry_run_rejects_an_untracked_file(tmp_path: Path) -> None:
    ...
    for name in (
        "launch_s4_r8_2gpu_tmux.sh",
        "s4_r8_runtime.py",
        "s0_hf_token_fifo.sh",
    ):
        shutil.copy2(ROOT / "scripts" / name, scripts / name)
    for name in ("prepare_s4_r8_shared.sh", "run_s4_r8_candidate.sh"):
        path = scripts / name
        path.write_text("#!/usr/bin/env bash\nexit 0\n", encoding="utf-8")
        path.chmod(0o755)

    subprocess.run(
        ["git", "init", "-b", "feat/model-improvements"], cwd=repo, check=True
    )
    subprocess.run(
    ...
    result = subprocess.run(
        [
            "bash",
            str(scripts / "launch_s4_r8_2gpu_tmux.sh"),
            "--run-id",
            "dirty",
            "--dry-run",
        ],
    ...
    assert not (repo / "outputs/s4_r8_runs/dirty").exists()
...
def test_stop_dry_run_resolves_only_four_manifest_windows(tmp_path: Path) -> None:
    ...
    tmux.write_text(
        ...
        "  printf 's4-r8-test-prepare|@1\\ns4-r8-test-p0|@2\\n'\n"
        "  printf 's4-r8-test-p1|@3\\ns4-r8-test-monitor|@4\\nother|@9\\n'\n"
        "  exit 0\n"
        "fi\n"
        "exit 0\n",
        encoding="utf-8",
    ...
    result = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts/stop_s4_r8_2gpu_tmux.sh"),
            "--run-id",
            "s4-r8-test",
            "--run-root",
            str(root),
    ...
    for suffix in ("prepare", "p0", "p1", "monitor"):
        assert f"window: s4-r8-test-{suffix}" in result.stdout
