...
def test_prepare_reuses_global_when_agent_camera_is_absent():
    from stereo_core.evaluate_no_wrist_pair import prepare_no_wrist_batch

    image = np.arange(4 * 5 * 3, dtype=np.uint8).reshape(4, 5, 3)
    observation = {
        "sensor_data": {"head_camera_global": {"rgb": image[None]}},
        "agent": {"panda-0": {"qpos": np.arange(9, dtype=np.float32)[None]}},
    }
    stats = {
        "q_mean": torch.arange(9, dtype=torch.float32),
        "q_std": torch.ones(9),
        "a_mean": torch.zeros(8),
        "a_std": torch.ones(8),
    }
    global_rgb, local_rgb, _qpos = prepare_no_wrist_batch(
        observation, (0,), stats, torch.device("cpu")
    )
    torch.testing.assert_close(global_rgb, local_rgb, rtol=0, atol=0)
