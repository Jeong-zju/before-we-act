...
from scripts.before_we_act.prepare_b3_n2_owner_closed_loop import (
    OWNER_TOKEN,
    PRIMARY_SERIES,
...
def test_owner_runner_preserves_old_conclusion_and_completes_validation20() -> None:
    ...
    launcher = (
        root / "scripts/before_we_act/run_b3_n2_owner_closed_loop.sh"
    ).read_text(encoding="utf-8")
    validation20 = (
        root / "scripts/before_we_act/run_b3_n2_validation20.sh"
    ).read_text(encoding="utf-8")

    assert OWNER_TOKEN in launcher
    assert 'OLD_CONCLUSION="${RUN_ROOT}/conclusion.json"' in launcher
    assert 'BWA_N2_CONCLUSION="${AUTHORIZATION}"' in launcher
    assert "run_b3_n2_validation20.sh" in launcher
    assert 'BWA_N2_CONCLUSION:-${RUN_ROOT}/conclusion.json' in validation20
