...
def test_in_memory_world_model_windows_match_disk_loader(tmp_path):
    paths = [
    ...
    try:
        ...
        for index in (0, 3, 4, 5, 9):
            ...
            for name in expected:
                ...
                torch.testing.assert_close(restored[index][name], expected[name])
    finally:
        disk.close()
        memory.close()
        restored.close()
...
def test_world_model_training_entrypoint_smoke(tmp_path):
